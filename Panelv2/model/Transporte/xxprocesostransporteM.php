	...
	//////fin maestros
	        ...
	        public function ValidarViaticos($Id_servicio,$n_item)
				 ...
				 public function ListarCabViaticos($Id_servicio,$n_item) //para el reporte viaticos
				{
					...
					{
						$this->pdo = Database::Conectar();  
						$stm = $this->pdo->prepare("select * from cabviaticos v,cabserv s where v.c_estado<>'0' and s.c_estado<>'0' and  v.Id_servicio=$Id_servicio and v.n_item=$n_item and s.Id_servicio=$Id_servicio"); 
						$stm->execute();
	...
	public function GuardarDetTransporte($data){
		try{
			...
			$sql = "insert into                                          
							...
							c_temp,c_venti,c_humedad,c_oxigeno,c_dioxido,c_codpurfresh,c_estado,c_kminicio,c_kmfin,c_kmtotal)                   
							values                   
							(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'1',?,?,?)";//15?
			$this->pdo->prepare($sql)
			->execute(
