...
from cabeot as c ,detaot as d where c.c_numot=d.c_numot  order by c.c_numot desc";
	 ...
	 public function GeneraNroOT()//genera el id de cotizacion
	{
		try
		{
			...
			$stm = $this->pdo->prepare("select max(c_numot) + 1 as cod from cabeot");
			$stm->execute();
	...
	public function GuardaDetOT($data)//guarda detalle de ot
	{
		...
		$sql="insert into detaot(c_numot,n_id,c_rucprov,c_nomprov,concepto,tdoc,ndoc,fdoc,monto,n_cant,n_igvd,n_totd,montop,c_tecnico,c_codconcepto
,c_subconcepto,
c_codsubconcepto,c_obsdoc)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public function GuardaCabOT($data)//guarda Cab de ot
	{
		...
		$sql="insert into cabeot(c_numot,c_codequipo,c_desequipo,c_idequipo,unidad,d_fecdcto,c_codmon,c_treal,c_asunto,c_supervisa,c_solicita,c_lugartab,c_ejecuta,c_cliente,d_fecentrega,c_usrcrea,d_fcrea,c_estado,c_refcot,c_osb)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			$this->pdo->prepare($sql)
			     ->execute(
				    array(                      
					    $data->c_numot,
