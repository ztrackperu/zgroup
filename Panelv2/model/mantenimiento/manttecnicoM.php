...
class Tecnicos
{
	...
	*/
	public function consultarTecnicos($params = array()){
		$error = true;
		$msg = '';
		$data = array();
		$sql = "SELECT C_CODTAB, C_NUMITM, C_DESITM  FROM dettabla WHERE c_estado = '1' AND C_CODTAB='UOT' ";
		if(!empty($params)){
			/*
			AQUI POSTERIORMENTE SE PUEDEN AGREGAR FILTROS A LA CONSULTA
			*/
		}
		$sql .= " ORDER BY C_NUMITM";
		try{
	...
	public function ObtieneTecnico($id)//
	{
	/* 	try
		{ */
			$result = array();
			$stm = $this->pdo->prepare("select * from dettabla where c_numitm='$id' and c_codtab='UOT'");
			$stm->execute();
