...
class Procesosinv
{
	...
	public function consultarTodosEquipos(){
		...
		$msg = '';
		$data = array();
		$sql = "SELECT  c_nronis, c_codsit, c_codsitalm, c_nserie, d_fecing, c_idequipo, IN_ARTI, c_refnaci, c_fisico, c_fisico2, COD_TIPO, c_motivoelim, doc.c_numeoc as orden_compra, 'INVENTARIO' as procedencia
						FROM ((invequipo e 
						...
						'' as c_fisico, '' as c_fisico2, COD_TIPO, '' as c_motivoelim, d.c_numeoc as  orden_compra, 'PENDIENTES' as procedencia
...
public function registratemcodigoM($data)
	...
	public function InventariarM($data){
		try 
		{
			$sql = "update invequipo set c_fisico=?,c_fisico2=?,usr_mod=?,fec_mod=? 
			where c_idequipo=? ";
	...
	function buscahistorialequipoM($idequipo){
		try{
				$sql = "SELECT DISTINCT PE_FDOC, p.pe_ncoti,c.c_numped,p.PE_NDOC,p.PE_CLIE,d.c_tipped, v.c_idequipo,d.c_codcont,v.PE_DESC,c.n_idasig,d.c_numeir,d.c_codcont, d.c_obsdoc
	...
	function buscaequipoeirM($idequipo){
        ...
        public function CambiarTipoNac($serieNA){
		try 
		{
			$sql = "UPDATE invequipo SET c_nacional='0' WHERE c_nserie='$serieNA' ";
        ...
        public function CambiarTipoInter($serieNA){
		try 
		{
			$sql = "UPDATE invequipo SET c_nacional='1' WHERE c_nserie='$serieNA' ";
