...
<script src="assets/js/busquedas/miscript.js"></script>-->
...
function salir(){
	...
	if(parseInt(cantFilas)>1){
		...
		//url volver  
		location.href="?c=not02&mod=<?php echo $_GET['mod']; ?>&udni=<?php echo $udni; ?>";	//&a=ListarNotas	
	//}
...
//maxDate: fecha tope D=días | M=mes | Y=año
  ...
  function validarguardar(){	  
	  var NT_FDOC=document.getElementById('NT_FDOC').value;
	   if(NT_FDOC==''){			
			var mensje = "Falta Ingresar Fecha de la Nota de Ingreso ...!!!";
				$('#alertone').modal('show');
				$('#mensaje').val(mensje);
	  ...
	  var NT_RESPO=document.getElementById('NT_RESPO').value;
	   if(NT_RESPO==''){			
			var mensje = "Falta Ingresar la persona Responsable de la Ingreso ...!!!";
				$('#alertone').modal('show');
				$('#mensaje').val(mensje);
	   ...
	   var NT_TREM=document.getElementById('NT_TREM').value;  
	   if(NT_TREM==''){			
			var mensje = "Falta Seleccionar Tipo de Dcto Referencia ...!!!";
				$('#alertone').modal('show');
				$('#mensaje').val(mensje);
			document.getElementById("NT_TREM").focus();
			return 0;
		}
	  
	  	var NT_SERIR=document.getElementById('NT_SERIR').value;  
	   if(NT_SERIR==''){			
			var mensje = "Falta Ingresar Serie de Dcto Referencia ...!!!";
				$('#alertone').modal('show');
				$('#mensaje').val(mensje);
			document.getElementById("NT_TREM").focus();
			return 0;
	  ...
	  //validar numero de Orden de Trabajo
	   ...
	   var NT_FRP=document.getElementById('NT_FRP').value;
	   if(NT_FRP==''){			
			var mensje = "Falta Ingresar Fecha de la Guia del Proveedor ...!!!";
				$('#alertone').modal('show');
				$('#mensaje').val(mensje);
			document.getElementById("NT_FRP").focus();
			return 0;
		}	
		
		var serieguia=document.getElementById('serieguia').value;
	   if(serieguia==''){			
			var mensje = "Falta Ingresar Serie de la Guia del Proveedor ...!!!";
				$('#alertone').modal('show');
				$('#mensaje').val(mensje);
			document.getElementById("serieguia").focus();
			return 0;
		}
		
		var numeroguia=document.getElementById('numeroguia').value;
	   if(numeroguia==''){			
			var mensje = "Falta Ingresar Numero de la Guia del Proveedor ...!!!";
				$('#alertone').modal('show');
				$('#mensaje').val(mensje);
	   ...
	   if(NT_FGUI==''){			
			var mensje = "Falta Ingresar Fecha de la Guia Transportista ...!!!";
				$('#alertone').modal('show');
				$('#mensaje').val(mensje);
		...
		var seriegt=document.getElementById('seriegt').value;
	   if(seriegt==''){			
			var mensje = "Falta Ingresar Serie de la Guia Transportista ...!!!";
				$('#alertone').modal('show');
				$('#mensaje').val(mensje);
			document.getElementById("seriegt").focus();
			return 0;
		}
		
		var numerogt=document.getElementById('numerogt').value;
	   if(numerogt==''){			
			var mensje = "Falta Ingresar Numero de la Guia Transportista ...!!!";
				$('#alertone').modal('show');
				$('#mensaje').val(mensje);
		...
		cantFilas = theTable.rows.length;
		if(cantFilas==1){
			mensje = "Falta Detalle de la Nota de Ingreso";
				$('#alertone').modal('show');
				$('#mensaje').val(mensje);	
			document.getElementById("c_desprd").focus();
			return 0;		
		}
		if(confirm("Seguro de Registrar la Nota de Ingreso ?")){
...
<!--fin modal alertas info-->   
 ...
 <form class="form-horizontal" id="Frmregcoti" name="Frmregcoti" method="post" action="?c=not02&mod=<?php echo $_GET['mod']; ?>&udni=<?php echo $udni; ?>&a=guardaNotaIngreso" >
 ...
 <ul class="nav nav-tabs" role="tablist">
    ...
    <li role="presentation"> <a  href="#transportista" aria-controls="transportista" role="tab" data-toggle="tab"  >Datos Guia Transportista</a></li>
  </ul> 
  
  <div class="tab-content">     
	<div role="tabpanel" id="cabecera"  class="tab-pane active"   >
      <!--<p><button data-user="1">Dame los datos de la persona con ID = 1</button> - 
       ...
       <input type="hidden" id="c_numeir" name="c_numeir" value="<?php echo $c_numeir; ?>"  />              
       </div>
       <!--fila 1-->
       <div class="form-group">
           <label class="control-label col-xs-2">Nro</label>
            ...
            <div class="col-xs-2">
             <input type="text" value="<?php echo $NT_FDOC ?>" id="NT_FDOC" name="NT_FDOC" class="form-control datepicker input-sm" placeholder="Fecha Documento" data-validacion-tipo="requerido" />  
            </div>
         	<label class="control-label col-xs-1">Tipo</label>
            <div class="col-xs-2">
              <input type="text" class="form-control input-sm"  value="NOTA DE INGRESO" readonly="readonly" data-validacion-tipo="requerido" />  
              <input type="hidden" id="NT_TDOC" name="NT_TDOC" value="I" class="form-control input-sm" data-validacion-tipo="requerido" />  
            </div>   
      </div>        
   <!--fila 2-->
   <div class="form-group">
          ...
          <input name="c_codalm" type="hidden" id="c_codalm" value="000001"   />         	
            ...
            <div class="col-xs-2">
             <select id="NT_TRAN" name="NT_TRAN"  class="form-control input-sm" o  >
                 <option value="">SELECCIONE</option>
                  <?php 
                    $listar=$this->model->ListartransacIngresoCompraM(); 
                    foreach ($listar as $listarval){
                    ?>
                  <option value="<?php echo $listarval->TT_CODI; ?>" selected="selected" ><?php echo utf8_encode($listarval->TT_DESC); ?></option>
                  <?php } ?>
			 </select>               
       	  </div>  
      ...
      <!--FILA 3-->
       <div class="form-group">
           <label class="control-label col-xs-2">Razon Social</label>
            <div class="col-xs-3">
             <input type="text" name="c_nomcli" id="c_nomcli"  class="form-control input-sm" value="<?php echo $c_nomprv; ?>" readonly="readonly" />
             <?php /*?><select id="xc_nomcli" name="xc_nomcli"  class="form-control input-sm" onchange="cambiacliente()"  >
                 <option value="">SELECCIONE</option>
                  <?php 
                    $ListarLineaMaritima=$this->maestro->listarProveedorfiltro(); 
                    foreach ($ListarLineaMaritima as $lineamar){
                    ?>
                  <option value="<?php echo utf8_encode($lineamar->datos); ?>" ><?php echo utf8_encode($lineamar->PR_RAZO); ?></option>
                  <?php } ?>
			 </select><?php */?>
            ...
            <div class="col-xs-2">
             <input type="text" id="NT_CCLI" name="NT_CCLI" value="<?php echo $c_codprv; ?>" class="form-control input-sm" placeholder="Codigo" readonly="readonly" />  
             
         </div>
         	<label class="control-label col-xs-1">Motivo Reg</label>
            <div class="col-xs-2">
              <input type="hidden" id="c_ruccli" name="c_ruccli" value="<?php echo $c_codprv; ?>" class="form-control input-sm" placeholder="Ruc" readonly="readonly" />  
              <input type="text" id="NT_RESPO" name="NT_RESPO" value="Orden de Compra" class="form-control input-sm" readonly="readonly"   />  
            </div>   
      </div>       
       
        <!--fila 4-->
        <div class="form-group">
        	<label class="control-label col-xs-2">OC/Transf.</label>
            <div class="col-xs-3">
             <input type="text" id="NT_NOC" name="NT_NOC"  class="form-control input-sm"  value="<?php echo $c_numeoc ?>" readonly="readonly" />              
        	</div> 
           <label class="control-label col-xs-1">Moneda</label>
            <div class="col-xs-2">
             <select name="NT_MONE" id="NT_MONE" class="form-control input-sm" > 
              ...
              <?php foreach($this->maestro->ListarMoneda() as $moneda):	 ?>                                 
                <option value="<?php echo $moneda->TM_CODI; ?>" <?php if($moneda->TM_CODI==$ListarCabOc->c_codmon){?>selected<?php }?> ><?php echo $moneda->TM_DESC; ?></option>
                <?php  endforeach;	 ?>
             </select> 
        	</div>    
             	  <?php
				  		 $xfecha=$ListarCabOc->d_fecoc;	
            ...
            <div class="col-xs-1">
              <input type="hidden" id="d_fecoc" name="d_fecoc" placeholder="Fecha Orden de Compra" value="<?php echo $fecha; ?>" /> 
        ...
        <div role="tabpanel" id="transporte"  class="tab-pane"   >  
           ...
           <label class="control-label col-xs-1"></label>
            ...
            <div class="form-group">
                <label class="control-label" style="margin-left:200px;">Datos del Documento de Referencia</label>
            </div>
               <!--fila 5-->
            <div class="form-group">
                ...
                <div class="col-xs-2">
                 <select name="NT_TREM" id="NT_TREM" class="form-control input-sm"  > 
                  <option value="">.:SELECCIONE:.</option> 
                  <?php foreach($this->model->ListarDocuIngresoM() as $docu):	 ?>                                 
                    <option value="<?php echo utf8_encode($docu->TD_CODI); ?>" <?php if(utf8_encode($docu->TD_CODI)=='M'){?>selected<?php }?> ><?php echo $docu->TD_DESC; ?></option>
                    <?php  endforeach;	 ?>
                 </select>  
                </div>
               <label class="control-label col-xs-1">Serie Dcto</label>
                <div class="col-xs-1">
                <!-- <select name="NT_SERIR" id="NT_SERIR" class="form-control input-sm"  > 
                  ...
                  <option value="001"  selected="selected">001</option>
                   ...
                   <input type="text" name="NT_SERIR" id="NT_SERIR" value="<?php echo $NT_SERIR; ?>" class="form-control input-sm" 
                   maxlength="3" onblur="ponerCerosserie(this)" onkeypress="return isNumberKey(event)"  /> 
                </div>                   
                ...
                <div class="col-xs-2">
                 ...
                 <input type="text" name="NT_DOCR" id="NT_DOCR" value="<?php echo $NT_DOCR; ?>" class="form-control input-sm" 
             ...
             <!--fila 6-->
            <div class="form-group">
                <label class="control-label" style="margin-left:200px;">Datos de la Guia del Proveedor</label>
            </div>
            <div class="form-group">
                <label class="control-label col-xs-2">Fecha Guia</label>
                <div class="col-xs-2">
                <input type="text" id="NT_FRP" name="NT_FRP" value="<?php echo $NT_FRP ?>" class="form-control datepicker input-sm" placeholder="Fecha Guia"  />
                </div>            
               <label class="control-label col-xs-1">Serie Guia</label>
                <div class="col-xs-1">
                 <!--<select name="serieguia" id="serieguia" class="form-control input-sm"  > 
                  <option value="">.:SELECCIONE:.</option> 
                  <option value="000">000</option>
                  <option value="001"  selected="selected">001</option>
                 </select> -->
                 	 <input type="text" name="serieguia" id="serieguia" value="<?php echo $serie; ?>" class="form-control input-sm" 
                  	 maxlength="3" onblur="ponerCerosserie(this)" onkeypress="return isNumberKey(event)"  /> 
                </div>                   
                <label class="control-label col-xs-1">Nro Guia</label>
                <div class="col-xs-2">
                 <!--<input type="text" id="NT_DOCR" name="NT_DOCR" class="form-control input-sm" placeholder="Nro Orden de Trabajo"
                 data-validacion-tipo="requerido" /> --> 
                 <input type="text" name="numeroguia" id="numeroguia" value="<?php echo $numero; ?>" class="form-control input-sm" 
                   maxlength="7" onblur="ponerCeros(this)" onkeyup = "compUsuario(event)" onkeypress="return isNumberKey(event)"  /> 
                   <!--<div id="DivDestino" style="width:150px">&nbsp;</div>
                    <input type="hidden" name="seguir" id="seguir" />-->
                </div>   
    ...
    <div role="tabpanel" id="transportista"  class="tab-pane"   >  
         ...
         <div class="form-group"> 
           <label class="control-label col-xs-2">Transportista </label>
            <div class="col-xs-4">
             <input autocomplete="off" type="text" name="c_nomtra" id="c_nomtra"  value="<?php echo $transportista; ?>" class="form-control input-sm" placeholder="Nombre o RUC"  />  
        	</div>
            <label class="control-label col-xs-1">RUC </label>
            <div class="col-xs-2">
             <input type="text" id="NT_CTR" name="NT_CTR" class="form-control input-sm"  value="<?php echo $NT_CTR; ?>" readonly="readonly"  /> 		
            </div> 
         ...
         <!--fila 7--> 
        <div class="form-group"> 
        	<label class="control-label col-xs-2">Fecha Guia Transportista</label>
            <div class="col-xs-2">
            <input type="text" id="NT_FGUI" name="NT_FGUI" value="<?php echo $NT_FGUI ?>" class="form-control datepicker input-sm" placeholder="Fecha Guia Transportista"  />
        	</div>
        	
            <label class="control-label col-xs-1">Serie</label>
                <div class="col-xs-1">
                 <!--<select name="seriegt" id="seriegt" class="form-control input-sm"  > 
                  <option value="">.:SELECCIONE:.</option> 
                  <option value="000">000</option>
                  <option value="001"  selected="selected">001</option>
                 </select> -->
                	 <input type="text" name="seriegt" id="seriegt" value="<?php echo $serie; ?>" class="form-control input-sm" 
                  	 maxlength="3" onblur="ponerCerosserie(this)" onkeypress="return isNumberKey(event)"  /> 
                </div>                   
                ...
                <div class="col-xs-2">
                 <!--<input type="text" id="NT_DOCR" name="NT_DOCR" class="form-control input-sm" placeholder="Nro Orden de Trabajo"
                 data-validacion-tipo="requerido" /> --> 
                 <input type="text" name="numerogt" id="numerogt" value="<?php echo $numero; ?>" class="form-control input-sm" 
                   maxlength="7" onblur="ponerCeros(this)" onkeyup = "compUsuario(event)" onkeypress="return isNumberKey(event)"  /> 
                   <!--<div id="DivDestino" style="width:150px">&nbsp;</div>
                    <input type="hidden" name="seguir" id="seguir" />-->
                </div> 
    ...
    </div> <!--end div  role="tabpanel" id="transportista"-->
            ...
            <label class="control-label col-xs-1">Observacion</label>
       		  ...
       		  <div class="row">                
                    <div class="col-xs-3">
                        ...
                        <input id="c_producto" name="c_producto" type="hidden" value="" />
                        <input id="IN_STOK" name="IN_STOK" type="hidden" value="" />
                        <input id="NT_PREC" name="NT_PREC" type="hidden" value="" /> IN_COST
                        <input  id="c_desprd" name="c_desprd" class="form-control input-sm" type="hidden"  />
...
<table  id="tblSample" class="table table-striped">
  ...
  <tbody>
  	<?php if($ListarDetOc!=NULL){
			$i=0;
		foreach($ListarDetOc as $detoc){
			$i++;
			$n_item=$detoc->n_item;
			$c_desprd=utf8_encode($detoc->c_desprd);
			$serie=$detoc->c_nroserie;
			if($serie!=""){
				$c_codcont=substr($detoc->c_desprd,0,1).'-'.$serie;
			}else{
				$c_codcont='';
			}
 	?>
  	...
  	<tr>
          <td><?php echo $n_item; ?><input id="<?php echo 'n_item'.$i ?>" name="<?php echo 'n_item'.$i ?>" type="hidden" value="<?php echo $n_item; ?>" /></td>
          <td><?php echo $detoc->c_codprd; ?><input id="<?php echo 'c_codprd'.$i ?>" name="<?php echo 'c_codprd'.$i ?>" type="hidden" value="<?php echo $detoc->c_codprd; ?>" /></td>
          <td><?php echo utf8_encode($detoc->c_desprd); ?></td>  
          <td><?php echo utf8_encode($detoc->c_obsdoc); ?><input id="<?php echo 'c_obsprd'.$i ?>" name="<?php echo 'c_obsprd'.$i ?>" type="hidden" value="<?php echo utf8_encode($detoc->c_obsdoc); ?>" /></td> 
          <td><?php echo $detoc->n_canprd; ?><input id="<?php echo 'n_canprd'.$i ?>" name="<?php echo 'n_canprd'.$i ?>" type="hidden" value="<?php echo $detoc->n_canprd; ?>" /></td>      
          <td><?php echo $serie; ?>
          	<input id="<?php echo 'c_codcont'.$i ?>" name="<?php echo 'c_codcont'.$i ?>" type="hidden" value="<?php echo $c_codcont ?>" />
          	<input id="<?php echo 'NT_PREC'.$i ?>" name="<?php echo 'NT_PREC'.$i ?>" type="hidden" value="<?php echo $detoc->n_preprd; ?>" />
          </td>           
          <td><?php echo $detoc->c_codund; ?><input id="<?php echo 'NT_CUND'.$i ?>" name="<?php echo 'NT_CUND'.$i ?>" type="hidden" value="<?php echo $detoc->c_codund; ?>" /></td>    
          <td><input type="button" name="button3" id="button3" value="delete"  class="btn btn-danger btn-sm" onClick="eliminarUsuario(this)" /></td>    
    		
    </tr>
    <?php } } ?>
    <input id="maxitem" name="maxitem" type="hidden" value="<?php echo $i; ?>"  />
  </tbody>
