...
function guardar() {
  ...
  if (cantFilas == 1) {
    ...
    $('#alertone').modal('show');
    $('#mensaje').val(mensje);
  } else if (document.Frmregcoti.fecha_despacho.value == "") {
    mensje = "Falta Fecha de despacho";
    $('#alertone').modal('show');
    $('#mensaje').val(mensje);
...
function recorregrid(){
	...
	//alert(document.getElementById("xc_meses").value)
	...
	var preapro=false;
	for(var y = 1;y <=100;y++){
    var c_desprd_selector = document.getElementById('c_desprd'+y);
    if(c_desprd_selector){
      var produ = c_desprd_selector.value;	
  ...
  if(preapro==true){
	  alert('Pre-Aprobado Ok');
	  }
}
</script>

<body onLoad="valcheckcrono()">
<form class="form-horizontal" id="Frmregcoti" name="Frmregcoti" method="post" action="?c=03&a=PreAprobarCotizacion&mod=<?php echo $_GET['mod']; ?>&udni=<?php echo $udni; ?>&alerta=1">
<!--modal fechas-->
<!-- Modal -->
...
<div class="modal fade" id="my_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
 ...
 //foreach($this->model->UltCotListar() as $item): ?>
  ...
  <input type="hidden" name="swfactura" id="swfactura" value="0">
  <?php
$fecha=date('d/m/Y');
...
<div class="panel panel-info">
  ...
  <div class="panel-heading">APROBACION DE COTIZACIONES.
 (NOTA SOLO GENERADA CRONOGRAMA A LOS ITEMS CON CLASE ALQUILER 017)<?php echo $valor."/".$sw;?></div>
  <div>
<div class="form-control-static" align="right">
<!--<a class="btn btn-default" onClick="recorregrid()" >Validar</a>&nbsp;-->
<input class="btn btn-success" type="button" onClick="recorregrid()" value="Validar"/>
&nbsp;
<input class="btn btn-success"  type="button" onClick="guardar()" value="PreAprobar."/>

<!--<a class="btn btn-success" onClick="guardar()">PreAprobar</a>-->&nbsp;
<!--<a class="btn btn-success" onClick="guardar()">Aprobar</a>&nbsp;-->
<a class="btn btn-danger" href="?c=03&a=AprCotizaciones&mod=<?php echo $_GET['mod']; ?>&udni=<?php echo $udni; ?>">Cancelar</a>&nbsp;
<a class="btn btn-warning" href="?c=03&a=AprCotizaciones&mod=<?php echo $_GET['mod']; ?>&udni=<?php echo $udni; ?>">Refrescar</a>&nbsp;
   ...
   <div class="form-group">
       ...
       <div class="form-group">
           <label class="control-label col-xs-1">Nro</label>
            <div class="col-xs-2">
             <input name="c_numped" type="text" class="form-control input-sm" id="c_numped" placeholder=" Nro autogenerado" value="<?php echo $c_numped ?>" readonly data-validacion-tipo="requerido" />  
        	<input type="hidden" name="login" id="login" value="<?php echo $login ?>  " /></div>                       
            ...
            <div class="col-xs-2">
              ...
              <?php foreach($this->maestro->ListarTipCot() as $a):	 ?>                                 
                <option value="<?php echo $a->tc_codi; ?>"<?php echo $c_tipped == $a->c_numitm ? 'selected' : ''; ?>> <?php echo $a->tc_desc; ?> </option>
                <?php  endforeach;	 ?>
             </select>	
   ...
   <!--fila 2-->
       <div class="form-group">
           <label class="control-label col-xs-1">Cliente</label>
            <div class="col-xs-3">
             <input name="c_nomcli" type="text" class="form-control input-sm" id="c_nomcli" placeholder="Cliente" autocomplete="off" value="<?php echo utf8_encode($c_nomcli) ?>" readonly/>  
        	</div>                     
       ...
       <div class="form-group"> 
           <label class="control-label col-xs-1">Contact</label>
            <div class="col-xs-3">
             <input name="c_contac" type="text" class="form-control input-sm" id="c_contac" placeholder="Contacto" value="<?php echo utf8_encode($c_contac) ?>" readonly data-validacion-tipo="requerido" />  
        	</div>                       
            <label class="control-label col-xs-1">Telf</label>
            <div class="col-xs-2">
             <input name="c_telef" type="text" class="form-control input-sm" id="c_telef" placeholder="Telefono" value="<?php echo $c_telef ?>" readonly data-validacion-tipo="requerido" />  
            
             </div>
         	<label class="control-label col-xs-1">Correo</label>
            <div class="col-xs-3">
            <input name="c_email" type="text" class="form-control input-sm" id="c_email" placeholder="Correo" value="<?php echo $c_email ?>" readonly data-validacion-tipo="requerido|email" /> 
        	</div>   
        </div>
        <!--fila 4-->
        <div class="form-group">
           <label class="control-label col-xs-1">Asunto</label>
            <div class="col-xs-4">
             ...
             placeholder="Asunto" value="<?php echo utf8_encode($c_asunto) ?>" readonly data-validacion-tipo="requerido" />  
        	</div>                       
			...
			<label class="control-label col-xs-1">Fecha de Despacho</label>
            <div class="col-xs-2">
              <input name="fecha_despacho" type="text" class="form-control datepicker input-sm" id="fecha_despacho" placeholder="Fecha Despacho" value="<?php echo vfecha(substr($fecha_despacho,0,10)) ?>"  data-validacion-tipo="requerido" 
               />
            </div>	
        </div>
</div><!--fin tab 1-->
...
<table  id="tblSample" class="table table-striped">
	...
	?>
    <tr>
      <th><input type="hidden" id="<?php echo "c_codprd".$i; ?>"  name="<?php echo "c_codprd".$i; ?>" readonly  value="<?php echo $itemD->c_codprd ?>" class='form-control input-sm' />
      <input type="checkbox"  name="<?php echo "re".$i;?>" id="<?php echo "re".$i;?>" 
 value="<?php echo $itemD->n_id;?>" onClick="limpiarvalida();"/>
      
      </th>

      <th><input  name="<?php echo "c_desprd".$i; ?>" type="text" id="<?php echo "c_desprd".$i; ?>" 
      value="<?php echo $itemD->c_desprd ?>"    class="form-control input-sm" size="40" readonly /></th>
      <th><input type="text" id="<?php echo "c_obsdoc".$i; ?>"  name="<?php echo "c_obsdoc".$i; ?>"   value="<?php echo utf8_encode($itemD->c_obsdoc); ?>" class="form-control input-sm" size="35" /></th>
      <th><input name="<?php echo "c_tipped".$i; ?>" size="4" type="text" id="<?php echo "c_tipped".$i; ?>"  value="<?php echo $itemD->clase; ?>" readonly class='form-control input-sm'/><input name="<?php echo "c_codcla".$i; ?>" type="hidden" id="<?php echo "c_codcla".$i; ?>"  value="<?php echo $itemD->c_codcla; ?>" readonly size="3" class='form-control input-sm'/></th>
      <th><input  name="<?php echo "c_codcont".$i; ?>" type="text" id="<?php echo "c_codcont".$i; ?>"  value="<?php echo $itemD->c_codcont ?>"  size="22" readonly class="form-control input-sm" /></th>
      <th><input name="<?php echo "c_fecini".$i; ?>" type="text" id="<?php echo "c_fecini".$i; ?>"   value="<?php echo $itemD->c_fecini;  ?>" class='form-control input-sm' onClick="abreVentanaF(this.name)"/></th>
      <th><input name="<?php echo "c_fecfin".$i; ?>" type="text" id="<?php echo "c_fecfin".$i; ?>"   value="<?php echo $itemD->c_fecfin;  ?>" class='form-control input-sm'/></th>
      <th> <input type="text" id="<?php echo "n_preprd".$i; ?>"  name="<?php echo "n_preprd".$i; ?>"   value="<?php echo $itemD->n_preprd; ?>" class='form-control input-sm' size="8" style="text-align:right">
    </th>
      <th> <input type="text" id="<?php echo "n_dscto".$i; ?>"  name="<?php echo "n_dscto".$i; ?>"  value="<?php echo $itemD->n_dscto; ?>" class='form-control input-sm' size="5" style="text-align:right">
</th>
      <th> <input type="text" id="<?php echo "n_canprd".$i; ?>"  name="<?php echo "n_canprd".$i; ?>"   value="<?php echo $itemD->n_canprd; ?>" onKeyUp="actualizar_importe(this.name)"  class='form-control input-sm' size="5" >
</th>
      <th> <input type="text" id="<?php echo "imp".$i; ?>"  name="<?php echo "imp".$i; ?>"   value="<?php echo $itemD->n_totimp; ?>" class='form-control input-sm' size="7" style="text-align:right"/>
</th>
      <th>
	  <?php
						if($itemD->FactuCoti!=''){
...
<table  class="table">
  <tr>
    ...
    <td width="101" align="right"><input name="n_bruto" id="n_bruto" type="text" class="form-control input-sm" size="5" readonly style="text-align:right"  value="<?php echo $n_bruto ?>"/></td>
  ...
  <tr>
    ...
    <td align="right"><input name="tn_dscto" id="tn_dscto" type="text" class="form-control input-sm" size="5" readonly style="text-align:right"  value="<?php echo $n_dscto ?>"/></td>
    <td align="right">&nbsp;</td>
  </tr>
  <tr>
    <td align="right">&nbsp;</td>
    ...
    <td align="right"><input name="n_neta" id="n_neta" type="text"  class="form-control input-sm" size="5" readonly style="text-align:right"  value="<?php echo $n_neta ?>"/></td>
    <td align="right">&nbsp;</td>
    </tr>
  <tr>
    <td align="right">&nbsp;</td>
    ...
    <div class="form-group">
       ...
       <div class="form-group">
           <label class="control-label col-xs-1">L.Entrega</label>
            <div class="col-xs-2">
             <input name="c_lugent" type="text" class="form-control input-sm" id="c_lugent" placeholder="Lugar de Entrega" value="<?php echo utf8_encode($c_lugent) ?>" readonly data-validacion-tipo="requerido" />  
        	</div>                       
            <label class="control-label col-xs-1">T.Entrega</label>
            <div class="col-xs-2">
             <input name="c_tiempo" type="text" class="form-control input-sm" id="c_tiempo" placeholder="Tiempo de  Entrega" value="<?php echo utf8_encode($c_tiempo) ?>" readonly data-validacion-tipo="requerido" />  
        	</div> 
       ...
       <div class="form-group">
           ...
           <label class="control-label col-xs-1">Precios</label>
            ...
            <div class="col-xs-2">
             <input name="c_validez" type="text" class="form-control input-sm" id="c_validez" placeholder="Validez de Oferta" value="<?php echo utf8_encode($c_validez) ?>" readonly data-validacion-tipo="requerido" />  
        	</div> 
            ...
            <div class="col-xs-2">
              <input type="text" name="c_numocref" id="c_numocref"  class="form-control input-sm" placeholder="Referencia Nro Dcto Cliente" data-validacion-tipo="requerido"  value="<?php echo $c_numocref ?>"/> 	
        	</div> 
        </div>
