...
/*function dias_transcurridos($fecha_i,$fecha_f)
{
	...
	return $dias;
}*/ //previously declared in D:\AppServ\www\Panelv2\model\funciones.php:724
...
$maxCuota = $cuota; 
$getCouta = $cuo;
foreach($this->model->ObtenerDataCotizacion($coti) as $r):
		$c_numped=$r->c_numped;
		$c_codmon=$r->c_codmon;
		$c_tipped=$r->c_tipped;
		$c_asunto=$r->c_asunto;
...
function agregardetalle(){
		...
		nuevaCelda=nuevaFila.insertCell(-1);
		nuevaCelda.innerHTML="<td><input name='n_dscto_"+posicionCampo+"' type='text'  id='n_dscto_"+posicionCampo+"'  size='10'  class='form-control input-sm' style='text-align:right'/>";
...
function guardar(){
	...
	if(cantFilas==1){
		...
		mensje = "Falta Ingresar Precios Con y/o Sin Igv";
			$('#alertone').modal('show');
			$('#mensaje').val(mensje);
		}else if((document.Frmregcoti.c_meses.value)=="0" || (document.Frmregcoti.c_meses.value)=="1" ){	
		mensje = "Número de meses incorrecto";
			$('#alertone').modal('show');
			$('#mensaje').val(mensje);			
	}else{
			if(confirm("Seguro de Ampliar Cronograma?")){
...
<body onLoad="cargar()">
<form class="form-horizontal" id="Frmregcoti" name="Frmregcoti" method="post" action="?c=05&a=GuardaAmpliCronoCoti&mod=<?php echo $_GET['mod']; ?>&udni=<?php echo $udni; ?>">
<!--modal fechas-->
<!-- Modal -->
...
<div class="panel panel-primary">
  ...
  <div class="panel-heading">AMPLIACION DE CRONOGRAMA
 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp (Tipo Cambio=<?php echo $tcambio ?>)<input name="n_tcamb"  id="n_tcamb" type="hidden" value="<?php echo $tcambio ?>" /></div>
  <div>
<div class="form-control-static" align="right">
<a class="btn btn-success" onClick="guardar()" href="#">Guardar</a>&nbsp;<a class="btn btn-danger" href="?c=03&a=AmpliaCronoCoti&mod=<?php echo $_GET['mod']; ?>&udni=<?php echo $udni; ?>">Cancelar</a>&nbsp;<a class="btn btn-warning" href="?c=05&a=AmpliaCronoCoti&mod=<?php echo $_GET['mod']; ?>&udni=<?php echo $udni; ?>">Refrescar</a>&nbsp;<a class="btn btn-info" href="indexa.php?c=05&mod=<?php echo $_GET['mod']; ?>&udni=<?php echo $udni; ?>">Salir</a>&nbsp;
</div>
       ...
       <div class="form-group">
           <label class="control-label col-xs-1">Nro</label>
            <div class="col-xs-2">
             ...
             <input name="c_cotipadre" type="hidden" id="c_cotipadre" value="<?php echo $_REQUEST['nrocoti']; ?>"/> 
             <input type="hidden" name="renovacion" id="renovacion" value="1">

             <input type="hidden" name="ultcouta" id="ultcouta" value="<?php echo $cuo ?>">
            
             <input type="hidden" name="nrocoti" id="nrocoti" value="<?php echo $coti ?>">
            </div>   
...
<table  id="tblSample" class="table table-striped">
	...
	//	echo 'aqui nrocot'.$_GET['ncoti'];
		foreach($this->model->ListaMaxCuotaDetCronograma($coti,(int)$cuo) as $itemD):
		/*$c_codprd=$r->c_codprd;	
		$ncoti=$c_numped;
		$n_item=$r->n_item;	 
		$tipo=$r->c_tipped;	*/
		//$i=$i+1;
                //echo var_dump($itemD->d_ffin) . "<br/>";

                
                
		//$i=0;
	//	echo 'aqui nrocot'.$_GET['ncoti'];
		//foreach($this->model->ObtieneDetCronograma($_REQUEST['nrocoti']) as $itemD):
		/*$c_codprd=$r->c_codprd;	
		$ncoti=$c_numped;
		$n_item=$r->n_item;	 
		$tipo=$r->c_tipped;	*/
		$i=$i+1;
                $fven=vfecha(substr($itemD->d_fecven,0,10));
                list($day,$mon,$year) = explode('/',$fven);
                    $ffin= date('d/m/Y',mktime(0,0,0,$mon,$day+30,$year));
                    
                list($day2,$mon2,$year2) = explode('/',$ffin);
                    $ffin2= date('d/m/Y',mktime(0,0,0,$mon2,$day2+29,$year2));
	?>
    
    <tr>
      <th>
      <?php //echo 're'.$i;?>
      <input type="hidden" id="<?php echo "re_".$i ?>"  name="<?php echo "re_".$i ?>" readonly  
      value="<?php echo $itemD->n_idreg ?>" class='form-control input-sm' />
      <input type="text" id="<?php echo "c_codprd_".$i ?>"  name="<?php echo "c_codprd_".$i ?>"   
      value="<?php echo $itemD->c_codprd ?>" class='form-control input-sm' /></th>

      <th><input  name="<?php echo "c_desprd_".$i ?>" type="text" id="<?php echo "c_desprd_".$i ?>" 
      value="<?php echo $itemD->c_desprd ?>"    class="form-control input-sm" size="40"  /></th>
      <th><input type="text" id="<?php echo "c_obsdoc_".$i ?>"  name="<?php echo "c_obsdoc_".$i ?>"   value="<?php echo 'DEL '.$ffin .' AL '.$ffin2; ?>" class="form-control input-sm" size="40" /></th>
      <th><input type="hidden" name="<?php echo "c_codcla_".$i ?>" id="<?php echo "c_codcla_".$i ?>" value="<?php echo $itemD->c_codcla ?>"></th>
      <th> <input name="<?php echo "c_tipped_".$i ?>" type="hidden" id="<?php echo "c_tipped_".$i ?>"  value="<?php echo $itemD->c_clase; ?>" readonly/>
&nbsp;</th>
      <th><input  name="<?php echo "c_codcont_".$i ?>" type="text" id="<?php echo "c_codcont_".$i ?>"  value="<?php echo $itemD->c_codequipo ?>"  size="18" readonly class="form-control input-sm" /></th>
      <th><input name="<?php echo "c_fecini_".$i ?>" type="text" id="<?php echo "c_fecini_".$i ?>"   
      value="<?php echo $ffin;   ?>" class='form-control input-sm' onClick="abreVentanaF(this.name)"/></th>
      <th><input name="<?php echo "c_fecfin_".$i ?>" type="text" id="<?php echo "c_fecfin_".$i ?>"  
       value="<?php echo $ffin2;  ?>" class='form-control input-sm'/></th>
      <th> <input type="text" id="<?php echo "n_preprd_".$i ?>"  name="<?php echo "n_preprd_".$i ?>"   value="<?php echo $itemD->n_importe; ?>" class='form-control input-sm' size="8" style="text-align:right">
    </th>
      ...
      <th> <input type="text" id="<?php echo "n_canprd_".$i ?>"  name="<?php echo "n_canprd_".$i ?>"   value="<?php echo $itemD->n_cant; ?>" onKeyUp="actualizar_importe(this.name)"  class='form-control input-sm' size="5" >
</th>
      <th> <input type="text" id="<?php echo "imp_".$i ?>"  name="<?php echo "imp_".$i ?>"  
       value="<?php echo (($itemD->n_importe) - ( $itemD->n_dscto) * ($itemD->n_cant)) ; ?>" class='form-control input-sm' size="7" style="text-align:right"/>
</th>
      <th><input type="button" name="button3" id="button3" value="delete"  class="btn btn-danger btn-sm"
       <?php /*?>onClick="eliminarUsuario(this)" <?php */?>/></th>
    </tr>
    <?php  endforeach; ?>  
    ...
    <div class="form-group">
       ...
       <div class="form-group">
         ...
         <div class="form-group">
           ...
           <input name="c_gencrono" id="c_gencrono" type="hidden" value="1" />          
            ...
            <div class="col-xs-2">
             ...
             <input name="c_meses" id="c_meses" type="hidden" value="0" />
        	</div> 
