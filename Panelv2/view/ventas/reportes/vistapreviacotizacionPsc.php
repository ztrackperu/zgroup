...
foreach($this->model2->ObtenerDataCotizacion($xc_numped) as $r):
		$c_numped=$r->c_numped;
		$c_codmon=$r->c_codmon;
		$c_tipped=$r->c_tipped;
		$c_asunto=$r->c_asunto;
		...
		$cantitems++;
endforeach;
if($this->model2->ObtenerDataUsuario($c_opcrea)!=NULL){
foreach($this->model2->ObtenerDataUsuario($c_opcrea) as $u):

$xc_opcrea=$u->c_login;

endforeach;
}
if($this->model2->ObtenerDataUsuario($c_uaprob)!=NULL){
foreach($this->model2->ObtenerDataUsuario($c_uaprob) as $a):
...
<table width="936" border="0" class="table table" >
  ...
  <tr>
	...
	//	echo 'aqui nrocot'.$_GET['ncoti'];
		foreach($this->model2->ObtenerDataCotizacion($xc_numped) as $itemD):
		$i=$i+1;
	?>
      <tr>
        ...
        <td bgcolor="#CCCCCC">
        <a href="indexinv.php?c=inv01&mod=<?php echo $_GET['mod']; ?>&udni=<?php echo $udni; ?>&a=ImprimirEquipo&id=<?php echo $itemD->c_codcont ?>&cod_tipo=<?php echo $itemD->c_codcla ?>" ><?php echo $itemD->c_codcont ?></a></td>
    ...
    <td height="27" colspan="6" ><table width="1008" border="0" class="table table">
      ...
      <tr>
        ...
        <td colspan="4"><span class="tablaImprimir" style="font-size:14px">
          <?php 
	foreach($this->model2->ListaCotiConFacturas($xc_numped) as $fac): 
	echo $fac->PE_NDOC.'|';
