...
$pdf->SetTextColor(24,44,78);
$pdf->cell(17,7,utf8_decode("COTIZACIÓN NRO:".$fila['c_numped']),0,0,'L');
 ...
 */
//$pdf->SetXY(5,76);
$pdf->MultiCell(199,6,'  ASUNTO :'.$asuntocot,1,'L','C','true'); 
$pdf->Ln(2); 

//$pdf->SetXY(5,85);
       
if($mostrar_cambio){
    $pdf->SetWidths(array(8,75, 18, 18,18, 18, 20, 24,22));
    $pdf->SetAligns(array('C','L','R','R','R','R','R','R','L'));
	
}else{
    $pdf->SetWidths(array(8,91, 20, 18,20, 18, 24,22));
...
for($i=0;$i<1;$i++){
    if($mostrar_cambio){
       ...
       // $pdf->Row(array('N','DESCRIPCION','PRECIO UNT.','DSCTO', 'TOTAL UNIT.', 'CANT.', 'PRECIO FINAL', 'PRECIO EN '.$simbolo_cambio));
		...
		$pdf->cell(8,7,'N',1,0,'C','true');
        $pdf->cell(91,7,utf8_decode('DESCRIPCIÓN'),1,0,'C','true');
...
while($resultado = odbc_fetch_array($historial)){
    ...
    $c_obsdoc = utf8_decode($resultado['c_obsdoc']); 
...
if(count($sep)>1){ 
    ...
    if($mostrar_cambio){
        ...
        $pdf->Row(array($n_item,$descripcionDet, $pre, $dscto, $totuni, $cant, $total, $total_al_cambio));
        ...
        $pdf->Row(array($n_item,$descripcionDet, $pre, $dscto, $totuni, $cant, $total));
/* 		$pdf->MultiCell(90,5,$descripcionDet."\n",1, 'J', 0, 0, '' ,'', true);
		//$pdf->Cell(18,5,".....",'LR',0,'C');
	   // $pdf->Cell(20,5,"----",'LR',0,'R');
		$pdf->Cell(15,5,$pre,1,0,'C',1);
...
if((int)$valSubt != 0){
		...
		$pdf->Ln(4);
		$pdf->SetFont('Flama-Basic','',9);
		$pdf->cell(33,4,'FORMA DE PAGO',0,0,'L');
		$pdf->cell(104,4,'  :  '.$fila['tp_desc'],0,0,'L');
        $pdf->cell(38,4,'DSCTO',1,0,'R','true');
        $pdf->cell(24,4,number_format($detdscto,2),1,0,'R');
		$pdf->Ln(4);
		$pdf->cell(33,4,'PRECIOS',0,0,'L');
		$pdf->cell(104,4,'  :  '.$mon,0,0,'L');
        $pdf->cell(38,4,'TOTAL',1,0,'R','true');
        $pdf->cell(24,4,number_format($dettotal,2),1,0,'R');
		$pdf->Ln(4);
		$pdf->cell(33,4,'TIPO DE MONEDA',0,0,'L');
		$pdf->cell(104,4,'  :  '.$moneda,0,0,'L');
		if($mostrar_cambio){
		...
		$pdf->Ln(4);
		if($mostrar_cambio){
			$pdf->cell(33,4,'TIPO DE CAMBIO',0,0,'L');
			$pdf->cell(104,4,'  :  '.$tipo_cambio,0,0,'L');
			$pdf->cell(38,4,'DSCTO '.$simbolo_cambio,1,0,'R','true');
			$pdf->cell(24,4,number_format($detdscto_al_cambio,2),1,0,'R');
			$pdf->Ln(4);
		}
		$pdf->cell(33,4,'TIEMPO DE ENTREGA',0,0,'L');
		$pdf->cell(104,4,'  :  '.utf8_decode($fila['c_tiempo']),0,0,'L');
		if($mostrar_cambio){
        ...
        $pdf->cell(24,4,'',0,0,'R');
		$pdf->Ln(4);
		$pdf->cell(33,4,'VALIDEZ DE OFERTA',0,0,'L');
		$pdf->cell(104,4,'  :  '.utf8_decode($fila['c_validez']),0,0,'L');
        $pdf->cell(38,4,'',0,0,'R');
        $pdf->cell(24,4,'',0,0,'R');
		$pdf->Ln(4);
		if($tipopedido=='020'){
			$pdf->cell(33,4,'LUGAR DE ENTREGA',0,0,'L');
			$pdf->cell(106,4,'  :  PACKING ZGROUP TAMBOGRANDE - APAGRO',0,0,'L');
			$pdf->cell(38,4,'',0,0,'R');
			$pdf->cell(24,4,'',0,0,'R');
		}elseif($tipopedido=='002' || $tipopedido=='001' || $tipopedido=='015' || $tipopedido=='019'){
			$pdf->cell(33,4,'LUGAR DE ENTREGA',0,0,'L');
			$pdf->cell(104,4,'  :  '.$html,0,0,'L');
			$pdf->cell(38,4,'',0,0,'R');
			$pdf->cell(24,4,'',0,0,'R');
			$html='  ALMACEN ZGROUP (MZ.D LTE. 14 PROGRAMA DE VIVIENDA ACUARIO, CALLAO (ALTURA PARADERO CARRILLO)) <A HREF="https://www.google.com.pe/maps/place/ZGROUP+SAC/@-11.9816981,-77.1247262,17z/data=!3m1!4b1!4m2!3m1!1s0x9105cdc090890995:0x9f144be70d5f6763">Google Maps</A>,';
		}elseif($tipopedido=='017'){			
			$pdf->cell(33,4,'LUGAR DE ENTREGA',0,0,'L');
			$pdf->cell(104,4,'  :  '.utf8_decode($fila['c_lugent']),0,0,'L');
			$pdf->cell(38,4,'',0,0,'R');
			$pdf->cell(24,4,'',0,0,'R');
		}
		if(isset($html))
...
$pdf->SetTextColor(255,255,255);//25,40,76
$pdf->SetFillColor(40,58,90); 
$pdf->MultiCell(199,6,'  BENEFICIOS',1,'L','C','true');
$pdf->SetFont('Flama-Basic','',9);
$pdf->SetTextColor(35,31,32);
$pdf->SetDrawColor(40,58,90);//COLOR DE LINEA TABLA
$pdf->SetFillColor(211,221,237);//COLOR TEXTO 
...
$pdf->SetTextColor(255,255,255);//25,40,76
$pdf->SetFillColor(40,58,90); 
$pdf->MultiCell(199,6,'  OBSERVACIONES / COMENTARIOS',1,'L','C','true');
$pdf->SetFont('Flama-Basic','',9);
$pdf->SetTextColor(35,31,32);
$pdf->SetFillColor(211,221,237);//COLOR TEXTO 
...
$pdf->Row(array(utf8_decode($resultado4)));
/**
...
$pdf->SetTextColor(255,255,255);//25,40,76
$pdf->SetFillColor(40,58,90); 
$pdf->MultiCell(199,6,'  CUENTAS CORRIENTES ZGROUP',1,'L','C','true');
$pdf->Ln(3); 
...
$pdf->SetFont('Flama-Basic','',9);
$pdf->cell(199,5,'Atentamente',0,1,'R');
$pdf->SetTextColor(40,58,90);//25,40,76
$pdf->SetFont('Flama-Bold','',12);
