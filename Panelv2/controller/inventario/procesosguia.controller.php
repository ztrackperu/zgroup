...
class ProcesosguiaController{
    ...
    public function ConsultarGuiaEspecifica(){	
            $ClienteG=$_REQUEST['BuscarNew'];
            
            $equi=$this->model->ListarGuiaXCliente($ClienteG);
            
            if($ClienteG==NULL){		
                  
		require_once 'view/principal/headerInicio.php';
		require_once 'view/principal/principal.php';
        require_once 'view/inventario/ConsultarGuiaEspecifica.php';
		require_once 'view/principal/footer.php';
            }else{
    ...
    public function ConsultarGuiaEspecifica2(){	
        $ClienteG = $_REQUEST['BuscarNew'];
        $equi=$this->model->ListarGuiaXCliente($ClienteG);
            if($ClienteG == ""){		
                //$mensaje="!!INGRESE UN NUMERO DE GUIA¡¡";
                print "<script>alert('$mensaje.XD.$ClienteG')</script>";	
                    require_once 'view/principal/headerInicio.php';
                    require_once 'view/principal/principal.php';
            require_once 'view/inventario/adminguia.php';
                    require_once 'view/principal/footer.php';
            }else{
                print "<script>alert('$ClienteG')</script>";	
                    require_once 'view/principal/headerInicio.php';
                    require_once 'view/principal/principal.php';
            require_once 'view/inventario/adminguia_1.php';
                    require_once 'view/principal/footer.php';
            }
    }
    ...
    public function InicioRegGuia(){			
		...
		//require_once 'view/inventario/ini_guia.php';
		require_once 'view/principal/footer.php';
	}//FIN InicioRegGuia
	public function InicioRegGuia2(){			
		require_once 'view/principal/headerInicio.php';
		require_once 'view/principal/principal.php';
		require_once 'view/inventario/inicioregguia.php';
		//require_once 'view/inventario/ini_guia.php';
	...
	public function EliminarGuiaNoregistrada(){
		...
		foreach ($secuencia as $item)
		{
			...
			if($verifica==NULL)
			{
				...
				$mensaje=" Guia registrada verifique......";
				print "<script>alert('$mensaje')</script>";		
			}
	    //listar
		require_once 'view/principal/headerInicio.php';
    ...
    {
        print_r(json_encode(
            $this->model->BuscarCotiAprobadas($_REQUEST['criterio']),JSON_UNESCAPED_UNICODE
        ));
	...
	public function GuardarRegGuia(){
		...
		foreach ($values as $key => $value) {
				if (strpos($key, 'c_desprd') === 0) {
					$indexes[] = substr($key,8);
				}
		}
		$alm = new Procesosguia();     
		//cabecera
		...
		//lcruzado
		//foreach($this->model->ObtenerIdGuia() as $guiaNro): 
		//	$GuiaNro = $guiaNro->maxguia+1;	
		//endforeach;
		//$c_nume=str_pad($GuiaNro, 7, '0', STR_PAD_LEFT); //lcruzado
		///lcruzado
		
		$alm->c_numguia=$c_numguia='E'.$_REQUEST['c_serdoc'].$c_nume;			
		$alm->c_tipdoc='E';
		$alm->c_serdoc=$_REQUEST['c_serdoc'];
		$alm->c_nume=$c_nume;
		$alm->d_fecgui=$_REQUEST['d_fecgui'];
		$alm->c_coddes=$_REQUEST['c_coddes'];
		$alm->campaña=$_REQUEST['campaña'];
		...
		$alm->sw_selva=isset($_REQUEST['sw_selva'])?$_REQUEST['sw_selva']:'';									
		...
		foreach ($indexes as $index) {
			$i = $index;
			if(isset($_REQUEST['c_desprd'.$i]) && $_REQUEST['c_desprd'.$i]!=""){  //si existe la fila	//if($_REQUEST['c_codprd'.$i]!=""){		
				$alm->c_numguia=$c_numguia='E'.$_REQUEST['c_serdoc'].$c_nume; //lcruzado
				...
				$alm->c_codcla = $c_codcla = $_REQUEST['c_codcla'.$i];
				if($alm->n_canprd==""){	 		
					$alm->n_canprd = 0;
				}
		...
		}else if($j==3){require_once 'view/inventario/Reportes/guias/I_imprimirguia3.php';
		}else if($j==4){require_once 'view/inventario/Reportes/guias/I_imprimirguia4.php';
		}else if($j==5){require_once 'view/inventario/Reportes/guias/I_imprimirguia5.php';
		}else if($j==6){require_once 'view/inventario/Reportes/guias/I_imprimirguia6.php';
		}else if($j==7){require_once 'view/inventario/Reportes/guias/I_imprimirguia6.php';
		}
		require_once 'view/principal/footer.php';
			 //VOLVER    				
    }//FIN GuardarRegGuia
...
public function ReporteGuiadetalladoexcel(){	
	...
	//require 'view/procesos/RepFiltProduccion.php';
}

public function ReporteEquiposexcel(){	
	$tipoexporta='EXCEL';
	$name='ReporteEquipos'.date('Y-m-d');
	if($tipoexporta=="EXCEL"){
		header("Content-type: application/vnd.ms-excel; name='excel'");
		header("Content-Disposition: filename=$name.xls");
		header("Pragma: no-cache");
...
//VerReporteListaEqSitu
	...
	endforeach;
			...
			}else if($i==3){require_once 'view/inventario/Reportes/guias/I_imprimirguia3.php';
			}else if($i==4){require_once 'view/inventario/Reportes/guias/I_imprimirguia4.php';
			}else if($i==5){require_once 'view/inventario/Reportes/guias/I_imprimirguia5.php';
			}else if($i==6){require_once 'view/inventario/Reportes/guias/I_imprimirguia6.php';
			}else if($i==7){require_once 'view/inventario/Reportes/guias/I_imprimirguia6.php';
			}else if($i==8){require_once 'view/inventario/Reportes/guias/I_imprimirguia6.php';
			}else if($i==9){require_once 'view/inventario/Reportes/guias/I_imprimirguia6.php';
			}else if($i==10){require_once 'view/inventario/Reportes/guias/I_imprimirguia6.php';
			}else if($i==12){require_once 'view/inventario/Reportes/guias/I_imprimirguia6.php';
			}else if($i==39){require_once 'view/inventario/Reportes/guias/I_imprimirguia39.php';
			}
	...
	public function ImprimirGuiaTransporte(){
		$guia=$_REQUEST['c_numguia'];
		 ...
		 public function validarNumGuia2(){//validar cuando ingresas un numero de guia
			 $xc_nume =$_REQUEST["cod"]; 
			 $serie=$_REQUEST["serie"];
			 $c_nume=str_pad($xc_nume, 8, '0', STR_PAD_LEFT);
			 if($c_nume!=''){
			 $validarCT=$this->model->ValidarGuiaM2($c_nume,$serie);
				if($validarCT != NULL){
					echo $mensaje= "<div class='alert_error'>Guia Ya Registrada</div>";
					//echo $seguir="0";
				}else{
...
//			require_once 'view/inventario/Reportes/verpdf.php';
			require_once 'view/inventario/guiaa4/test.php';
		//	require_once 'view/principal/footer.php';
			...
			public	function RegistroGuiaTransporte(){
