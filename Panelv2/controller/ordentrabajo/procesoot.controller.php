...
//lista codigo productos
	...
	public function GuardarOT(){
		...
		//foreach($this->model->GeneraNroOT() as $correlativo):
			 $GeneraNroOT = $this->model->GeneraNroOT();
			 // 
			  $c_numot=$GeneraNroOT->cod+1;	
		...
		$Guardacabot = new OrdenTrabajo();
						...
						$c_idequipo=utf8_decode(mb_strtoupper($_REQUEST['c_codcont']));
						$Guardacabot->c_idequipo=$c_idequipo;
						$Guardacabot->unidad=$c_idequipo;
						...
						$Guardacabot->c_codsolicita=utf8_decode(mb_strtoupper($_REQUEST['c_supervisa']));
						$Guardacabot->c_codsupervisa=utf8_decode(mb_strtoupper($_REQUEST['c_solicita']));
				...
				for($i=1;$i<=200;$i++){
						...
						if($_REQUEST['c_rucprov'.$i]!=NULL){
						$this->model->GuardaDetOT($GuardaDetot);
						}
						
						$updEquipoOT = new OrdenTrabajo();
						$updEquipoOT->c_codsit="M";
						$updEquipoOT->c_codsitalm="M";
						$updEquipoOT->c_idequipo=$c_idequipo;
						$this->model->UpdateEstEquiOT($updEquipoOT);	
