...
function ListarComprobantesM($fecha,$c_codasi,$vou,$anno,$mes){
	...
	$sql="SELECT * FROM VOUMAE   where  c_codasi='$c_codasi' and c_anovou='$anno' and c_mesvou='$mes' and c_numvou='$vou'  ORDER BY c_numvou DESC";	
	$resultado = odbc_exec($cid, $sql)or die("Error: ".odbc_error($cid));
	while ($fila=odbc_fetch_array($resultado))
	{
	$ven[] = $fila;
...
function UpdateComprobanteM($c_numvou,$c_numeOP,$c_anovou,$c_mesvou,$c_NumOT){
include('cn/dbconex.php');
$sql="update voumae set c_tipOP='C',c_swtOP='S',c_numeOP='$c_numeOP',c_NumOT='$c_NumOT' where c_numvou='$c_numvou'
...
function BuscarFacMPs($nro)	{ //tabla voumae
...
function updatetc1M($tc,$tipodoc,$serie,$docu){
	include('cn/dbconex.php');
	$sql="update pefmae set PE_TCAM=$tc  where PE_TDOC='$tipodoc' and PE_SERI='$serie' and PE_DOC='$docu' and PE_ESTA<>'4'";
	$resultado = odbc_exec($cid, $sql)or die(exit("Error en odbc_exec()<br>$sql"));
	return $resultado;
	}
function updatetc2M($tc,$tipodoc,$serie,$docu){
	include('cn/dbconex.php');
	$sql="update t_pefmae set PE_TCAM=$tc where PE_TDOC='$tipodoc' and PE_SERI='$serie' and PE_DOC='$docu'  and PE_ESTA<>'4'";
	$resultado = odbc_exec($cid, $sql)or die(exit("Error en odbc_exec()<br>$sql"));
	return $resultado;
	}
function updatetc3M($tc,$tipodoc,$serie,$docu){
	include('cn/dbconex.php');
	$sql="update climov set CC_TCAM=$tc   where CC_TDOC='$tipodoc' and CC_SERI='$serie' and CC_DOC='$docu'";
	$resultado = odbc_exec($cid, $sql)or die(exit("Error en odbc_exec()<br>$sql"));
	return $resultado;
	}
	
function updatetc1MPs($tc,$tipodoc,$serie,$docu){
	include('cn/dbconex_ps.php');
	$sql="update pefmae set PE_TCAM=$tc  where PE_TDOC='$tipodoc' and PE_SERI='$serie' and PE_DOC='$docu' and PE_ESTA<>'4'";
	$resultado = odbc_exec($cid, $sql)or die(exit("Error en odbc_exec()<br>$sql"));
	return $resultado;
	}
function updatetc2MPs($tc,$tipodoc,$serie,$docu){
	include('cn/dbconex_ps.php');
	$sql="update t_pefmae set PE_TCAM=$tc where PE_TDOC='$tipodoc' and PE_SERI='$serie' and PE_DOC='$docu'  and PE_ESTA<>'4'";
	$resultado = odbc_exec($cid, $sql)or die(exit("Error en odbc_exec()<br>$sql"));
	return $resultado;
	}
function updatetc3MPs($tc,$tipodoc,$serie,$docu){
	include('cn/dbconex_ps.php');
	$sql="update climov set CC_TCAM=$tc   where CC_TDOC='$tipodoc' and CC_SERI='$serie' and CC_DOC='$docu'";
	$resultado = odbc_exec($cid, $sql)or die(exit("Error en odbc_exec()<br>$sql"));
	return $resultado;
...
function guardamodtcMPs($usuario,$fechamod,$documento,$tipocambio){
	include('cn/dbconex_ps.php');
	$sql="insert into cambiostc (usuario,fechamod,documento,tipocambio)
values ('$usuario','$fechamod','$documento','$tipocambio')";
	$resultado = odbc_exec($cid, $sql)or die(exit("Error en odbc_exec()<br>$sql"));
