...
class Procesostransporte
{
	...
	{         
	   ...
	   public function ListarPuertos() 
		...
		public function ListarDepositos() //DEPOSITOS,almacen
		{
			...
			public function ListarTipoDocumentoCont($idtipdoc)
				{
					try
					{
						$this->pdo = Database::Conectar();  
	...
	//////fin maestros
	        ...
	        public function ValidarViaticos($Id_servicio,$n_item)
				 ...
				 public function ListarCabViaticos($Id_servicio,$n_item) //para el reporte viaticos
				{
					...
					{
						$this->pdo = Database::Conectar();  
						$stm = $this->pdo->prepare("select * from cabviaticos v,cabserv s where v.c_estado<>'0' and s.c_estado<>'0' and  v.Id_servicio=$Id_servicio and v.n_item=$n_item"); 
						$stm->execute();
				 ...
				 public function ListarDetViaticos($Id_servicio,$n_item)
				{
					...
					{
						$this->pdo = Database::Conectar(); 
						$stm = $this->pdo->prepare("select * from cabviaticos c,detviaticos d where c.Id_viatico=d.Id_viatico and c.c_estado<>'0' and d.c_estado<>'0'   
						and  c.Id_servicio=$Id_servicio and n_item=$n_item");	
						$stm->execute();			
				...
				public function ListarLiquidar($Id_detviatico)
				{
					...
					{
						$this->pdo = Database::Conectar();  
						$stm = $this->pdo->prepare("select l.*,d.c_moneda,d.n_importe,d.n_impogastot from liquidetviaticos l left join detviaticos d on l.Id_detviatico=d.Id_detviatico
													where d.c_estado='1' and l.c_estado='1' and l.Id_detviatico=$Id_detviatico order by Id_liquidetviatico asc"); 
						$stm->execute();
	...
	public function GuardarLiquidacion($data) //guardar  liquidetviaticos y sumar n_impogastot en detviaticos
	{
		try 
		{
			$this->pdo = Database::Conectar();  
	...
	public function BuscarCotiAprobadas($criterio)//Buscar todas las cotizacion  FILTRO
	{         
		try
		{
			$this->pdo = Database::Conectar();  
			$stm = $this->pdo->prepare("SELECT c_numped,c_nomcli,c_numped&' '& c_nomcli as cliente,c_codcli,CC_NRUC from pedicab,climae
			where c_codcli=CC_RUC and  c_estado<>'4' and c_numped LIKE '%$criterio%' order by c_numped desc "); //and n_swtapr<>0
			$stm->execute();
			return $stm->fetchAll(PDO::FETCH_OBJ);
		}
		catch(Exception $e)
	...
	public function BuscarPeaje($criterio)
	{
		...
		{
			$this->pdo = Database::Conectar();  
			$stm = $this->pdo->prepare("SELECT c_codpeaje,c_descripcion&' '&c_provincia as descripcion  from peajes where c_estado='1'  and (c_descripcion like '%$criterio%'  or c_provincia like '%$criterio%') ORDER BY c_descripcion asc"); 
			$stm->execute();
	...
	public function ObtenerItemServicioLocal($Id_servicio) //obtener item del servicio Local
	{
		try
		{
			$this->pdo = Database::Conectar();  
	...
	}//fin  ObtenerItemServicioLocal
	
	public function ValidarDetServicioEquipo($c_numequipo,$c_tipmov,$Id_servicio) //valida que no se registren equipos iguales
	{
		try
		{
			$this->pdo = Database::Conectar(); 
			if($c_tipmov=='I'){ 
				$stm = $this->pdo->prepare("select * from detservimpo where c_estado<>'0' and Id_servicio=$Id_servicio and c_numequipo='$c_numequipo'"); 
			}else if($c_tipmov=='E'){
				$stm = $this->pdo->prepare("select * from detservexpo where c_estado<>'0' and Id_servicio=$Id_servicio and c_numequipo='$c_numequipo'"); 
			}else if($c_tipmov=='L'){
				$stm = $this->pdo->prepare("select * from detservlocal where c_estado<>'0' and Id_servicio=$Id_servicio and c_numequipo='$c_numequipo'"); 
			}
			$stm->execute();
	...
	public function GuardarCabTransporte($data)
	{
		try 
		{
			$this->pdo = Database::Conectar();  
			$sql = "insert into
					cabserv(Id_servicio,c_nrofw,c_nomcli,c_codcli,c_ruccli,c_numped,d_fectran,d_fecreg,c_usureg,c_tipmov,c_estado)
					values (?,?,?,?,?,?,?,?,?,?,'1')"; 
	...
	public function GuardarDetTransporteExpo($data)
			...
			$this->pdo = Database::Conectar();  
			$sql = "insert into
					detservlocal(Id_servicio,	c_numped,	n_item,	c_numguia,	c_rucclie,	c_nomclie,	c_contactocli,	
					...
					c_placa,	c_placa2,	c_telefono,	c_diresal,	c_direlle,	c_observacion,	c_usucrea,	d_fecreg,	c_estado)
					values (?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	?,	'1')";
			   ...
			   public function GuardarCabViaticos($data)
				...
				public function GuardarDetViaticos($data)
				{
					try 
					{
						$this->pdo = Database::Conectar();  
						$sql = "insert into
								detviaticos(Id_viatico,c_ususolic,c_usuaut,c_personal,c_idconcepto,c_nomconcepto,c_descripcion,c_coddes,d_fecsol,c_tipo,c_moneda,n_importe,n_saldo,c_refdeposito,d_fecdeposito,d_fecreg,c_usureg,c_estado)
								values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'1')"; 
				...
				public function GuardarUpdDetViaticosAlm($data)
		{
			try 
			{
				$this->pdo = Database::Conectar();  
				$sql = "update detviaticos
						set c_ususolic=?,c_usuaut=?,c_personal=?,c_idconcepto=?,c_nomconcepto=?,c_descripcion=?,c_coddes=?,
		   ...
		   public function EliminarDetViaticos($Id_detviatico,$c_userelim,$d_fecelim)
			...
			public function EliminarViaticos($Id_servicio,$c_userelim,$d_fecelim)
                {
				   try 
				   {
				   $this->pdo = Database::Conectar(); 				   
				   $sql = "update cabviaticos set c_estado='0',c_userelim='$c_userelim',d_fecelim='$d_fecelim' where Id_servicio=$Id_servicio";
				   $this->pdo->prepare($sql)
						->execute();
					   } catch (Exception $e) 
					   {
	...
	//////////////editar transporte
	public function ListarDetalleUpd($c_tipmov,$Id_servicio,$n_item) //listado de Detalle de Servicio para editar
	{
		try
		{
			$this->pdo = Database::Conectar(); 
			if($c_tipmov=='I'){ 
				$stm = $this->pdo->prepare("select * from ((cabserv c left join detservimpo d on  c.Id_servicio=d.Id_servicio)
											 left join detserv s on d.Id_servicio=s.Id_servicio )
											where d.c_estado<>'0'  and s.c_estado<>'0' and d.Id_servicio=$Id_servicio and d.n_item=$n_item and s.n_item=$n_item"); 
			}else if($c_tipmov=='E'){ //,TimeValue (d_fecretiro)as horaretiro,TimeValue (d_fecingreso)as horaingreso
				$stm = $this->pdo->prepare("select * from ((cabserv c left join detservexpo d on  c.Id_servicio=d.Id_servicio)
											 left join detserv s on d.Id_servicio=s.Id_servicio )
											where d.c_estado<>'0'  and s.c_estado<>'0' and d.Id_servicio=$Id_servicio and d.n_item=$n_item and s.n_item=$n_item"); 
			}else if($c_tipmov=='L'){
				$stm = $this->pdo->prepare("select * from ((cabserv c left join detservlocal d on  c.Id_servicio=d.Id_servicio)
											 left join detserv s on d.Id_servicio=s.Id_servicio )
											where d.c_estado<>'0'  and s.c_estado<>'0' and d.Id_servicio=$Id_servicio and d.n_item=$n_item and s.n_item=$n_item"); 
			}
			$stm->execute();
	...
	public function GuardarUpdDetTransporteExpo($data)
			...
			$sql = "update detservlocal
					set c_numped=?,		c_numguia=?,	c_rucclie=?,	c_nomclie=?,	c_contactocli=?,	
					c_seriefac=?,	c_numerofac=?,	d_fecfac=?,	c_eirconten=?,	c_desconten=?,	c_numequipo=?,	c_codtiprod=?,	
					c_tamequipo=?,	c_eirgen=?,	c_desgen=?,	c_numgen=?,	c_nrotransp=?,	c_guiatranslleno=?,	d_fecguiatranslle=?,	
					c_guiatransvacio=?,	d_fecguiatransva=?,	c_ructransporte=?,	c_nomtranspote=?,	c_chofer=?,	c_licenci=?,	c_marca=?,	
					c_placa=?,	c_placa2=?,	c_telefono=?,	c_diresal=?,	c_direlle=?,	c_observacion=?,	c_userupd=?,	d_fecupd=?	
		...
		public function GuardarUpdDetTransporte($data)
                {
				   ...
				   $this->pdo = Database::Conectar();  
				   $sql = "update detserv
				   set c_precivacio=?,c_preciaduana=?,c_precilinea=?,c_precilinea2=?,c_precilinea3=?,c_precizgroup=?,c_codterm1=?,c_codterm2=?,
                   c_temp=?,c_venti=?,c_humedad=?,c_oxigeno=?,c_dioxido=?,c_codpurfresh=?                   
                   where Id_servicio=? and n_item=?  and c_estado<>'0' ";//13?
				   $this->pdo->prepare($sql)
						->execute(
							 array( 
							 //$data->Id_servicio,
	...
	{         
		...
		{
			$this->pdo = Database::Conectar();  
			$stm = $this->pdo->prepare("select * from ((pedicab c left join cabguia g on c.c_numguia=g.c_numguia)
										left join climae cli on  c.c_codcli=cli.CC_RUC)
										where c.c_Estado<>'4' and c.c_numped='$c_numped' ");										
			$stm->execute();
			return $stm->fetch(PDO::FETCH_OBJ);
		}
		catch(Exception $e)
	...
	{         
		...
		{
			$this->pdo = Database::ConectarSql();  
			$stm = $this->pdo->prepare("select distinct Fwd_Codi,Fwd_Tmov, 
	...
	{         
		...
		{
			$this->pdo = Database::ConectarSql();  
			$stm = $this->pdo->prepare("select distinct Fwd_Codi,Fwd_Bkng,Fwd_Tmov, 
							Fwd_Csne,Ent_Rsoc,Fwd_Inco,Fwd_NBL1,Fwd_NBL2,Fwd_Lin1,Lin_Desc
							,Vje_Nave,Nav_Desc,Vje_Nvia,
							CONVERT(VARCHAR(10), Vje_Fzar,103)as Vje_Fzar,CONVERT(VARCHAR(10), Vje_Farr,103)as Vje_Farr,
							Fwd_NroManifiesto,Fwd_Cemb,
