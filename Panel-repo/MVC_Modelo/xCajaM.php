...
and c.c_numguia=d.c_numguia order by c.c_numguia desc";
...
function listaequipoSituacionFiltroM($cod){
		include('cn/dbconex.php');
		$sql="SELECT
    invequipo.c_idequipo, invequipo.c_nserie, invequipo.d_fecing,c_codsitalm, invequipo.c_nrodoc, invequipo.c_oper,c_docpdf,
...
$sql="SELECT
    invequipo.c_idequipo, invequipo.c_nserie,c_codsitalm, invequipo.d_fecing, invequipo.c_nrodoc, invequipo.c_oper,c_docpdf,
    Invmae.IN_CODI, Invmae.IN_ARTI, Invmae.tp_codi,cod_tipo,
    Q_Situacion_Equipo.C_ABRITM,
    q_Clase_Producto.C_DESITM
FROM
...
function actualizaprovM($PR_RUC,$PR_RAZO,$PR_DIR1,$PR_DIR2,$PR_TELE,$PR_EMAI){
	include('cn/dbconex.php');
	
$sql="update promae set PR_RAZO='$PR_RAZO',PR_DIR1='$PR_DIR1',PR_DIR2='$PR_DIR2',PR_TELE='$PR_TELE',PR_EMAI='$PR_EMAI' where pr_ruc='$pr_ruc'";
		$resultado = odbc_exec($cid, $sql)or die(exit("Error en odbc_exec()<br>$sql"));
	return $resultado;
}
...
function listar_CotizacionesAsunto($asunto){
	include('cn/dbconex.php');
...
where  c.c_numped=d.c_numped and  d.c_desprd like '%$asunto%' order by c.c_numped desc";
	$resultado = odbc_exec($cid, $sql)or die("Error: ".odbc_error($cid));
	while ($fila=odbc_fetch_array($resultado))
	{
	$ven[] = $fila;
...
function Listar_CotizacionesM($cli,$ff,$sw){
...
function Listar_CotizacionesxNroM($cli,$ff,$sw){
...
c_precios,c_tiempo,c_validez,c_desgral,c_tipocoti,b_IncIgv,c_codtit,c_gencrono,c_swfirma) 
...
'$c_precios','$c_tiempo','$c_validez','$c_desgral','$c_tipocoti',$b_inlgv,'$c_codtit','$c_gencrono','$c_swfirma')";
...
c_oper='$c_oper',c_precios='$c_precios',c_tiempo='$c_tiempo',c_validez='$c_validez',c_desgral='$c_desgral',b_IncIgv=$b_inlgv ,c_codtit='$c_codtit',c_gencrono='$c_gencrono',c_swfirma='$c_swfirma',c_meses='$c_meses',c_aprvend='$c_aprvend' where c_numped='$c_numped'";
...
include('cn/dbconex.php');
//and c.c_estado='0' 
		$sql="SELECT d_finicio,c.c_numped,c_nomcli,c_codcli,c_asunto,d.c_numped as pedido,c_nroped as ped,n_importe,d_fecven,c_nrofac,c_nroped,d.n_idreg,c_codprd,c_desprd,c_codequipo,d_fecapr,c_meses from pedi_cronograma as  d, pedicab as c where  Left ( d.c_numped,11 )=c.c_numped and Left ( d.c_numped,11 )='$coti' and c.c_estado<>'4' order by d_fecven asc";	
	 $resultado = odbc_exec($cid, $sql)or die("Error: ".odbc_error($cid));
	while ($fila=odbc_fetch_array($resultado))
	{
	$ven[] = $fila;
...
function UpdateCotizacionCabM($coti,$fpag,$d_fecvig,$useraprob,$d_fecapr){ //B_INCIGV
	include('cn/dbconex.php');
	$sql="update pedicab set c_estado='0',c_aprvend='1', n_swtapr='1',d_fecapr='$d_fecapr' ,c_uaprob='$useraprob', C_CODPGA='$fpag',C_CODPGF='$fpag', d_fecvig='$d_fecvig' where c_numped='$coti'  ";
	$resultado = odbc_exec($cid, $sql)or die(exit("Error en odbc_exec()<br>$sql"));
	return $resultado;
		}
...
function listaritemM($chk){
		include('cn/dbconex.php');
		$sql="select * from pedidet where c_numped='$chk'";		
		$resultado = odbc_exec($cid, $sql)or die("Error: ".odbc_error($cid));
		while ($fila=odbc_fetch_array($resultado))
		{
		$ven[] = $fila;
