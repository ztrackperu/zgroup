...
//$sql="SELECT c_codprd from precio where  c_codprd='$c_codprd' and c_catprd='$c_catprd' and c_conprd='$c_conprd' and c_estado='1' and c_activo='1'";
	...
	function BusquedasiexisteProductoPrecioM($c_codprd,$c_catprd,$c_conprd,$n_preprd,$c_codmon,$c_mcamaq,$c_cmodel){
include('cn/dbconex.php');		
	
			
$sql="SELECT c_numpre from precio where c_codprd='$c_codprd' and c_catprd='$c_catprd' and c_conprd='$c_conprd' and n_preprd=$n_preprd and c_codmon='$c_codmon' and c_mcamaq='$c_mcamaq' and c_cmodel='$c_cmodel' ";
	...
	function DesactivarUltimoProductoM($c_codprd,$c_catprd,$c_conprd,$c_opermod,$d_fecmod,$c_mcamaq,$c_cmodel){
	include('cn/dbconex.php');
	$sql="update precio set c_activo='0',c_opermod='$c_opermod',d_fecmod='$d_fecmod' where c_activo='2' and c_estado='1' 
	and c_codprd='$c_codprd' and c_catprd='$c_catprd' and c_conprd='$c_conprd' and c_mcamaq='$c_mcamaq' and  c_cmodel='$c_cmodel' ";
	$resultado = odbc_exec($cid, $sql)or die(exit("Error en odbc_exec()<br>$sql"));
	return $resultado;	
    }
	
	function DesactivarProductoActivadoM($c_codprd,$c_catprd,$c_conprd,$c_opermod,$d_fecmod,$c_mcamaq,$c_cmodel){
	include('cn/dbconex.php');
	...
	function ActivarProductoM($c_codprd,$c_catprd,$c_conprd,$n_preprd,$c_opermod,$d_fecmod,$c_mcamaq,$c_cmodel){
	include('cn/dbconex.php');
	$sql="update precio set c_activo='1',c_estado='1',c_opermod='$c_opermod',d_fecmod='$d_fecmod' 
	where c_codprd='$c_codprd' and c_catprd='$c_catprd' and c_conprd='$c_conprd' and n_preprd=$n_preprd and c_mcamaq='$c_mcamaq' and  c_cmodel='$c_cmodel'";
	$resultado = odbc_exec($cid, $sql)or die(exit("Error en odbc_exec()<br>$sql"));
	return $resultado;	
    }
...
$resultado = odbc_exec($cid, $sql)or die("Error: ".odbc_error($cid));
	...
	function ListarModeloPorMarca($id){
	include('cn/dbconex.php');
	$sql="SELECT C_NUMITM, C_DESITM, C_ESTADO
...
$resultado = odbc_exec($cid, $sql)or die("Error: ".odbc_error($cid));
	...
	function UpdPrecioNewM($c_numpre,$c_mcamaq,$c_cmodel){
	include('cn/dbconex.php');
	$sql="update precio set c_mcamaq='$c_mcamaq',c_cmodel='$c_cmodel' where c_numpre='$c_numpre'";
	$resultado = odbc_exec($cid, $sql)or die(exit("Error en odbc_exec()<br>$sql"));
	return $resultado;	
	}	
	
	//POR PRODUCTO
		function listahistorialGeneralM($f1,$f2,$sw){
		include('cn/dbconex.php');				
		
		if($sw=='2'){
			$sql="select * from precio where c_estado='1' and c_activo='1'  and  d_fecreg Between #$f1# and #$f2# order by d_fecreg desc";	
		}
		
		if($sw=='1'){
			$sql="select * from precio where c_estado='1' and c_activo='1' order by c_numpre desc";
		}				
	...
	function ListaMarcaMaqDesM($cod){
	include('cn/dbconex.php');
	$sql="SELECT Dettabla.C_NUMITM, Dettabla.C_DESITM, Dettabla.C_ESTADO
FROM Dettabla
WHERE Dettabla.C_CODTAB='MCM' and Dettabla.C_NUMITM='$cod' ";
$resultado = odbc_exec($cid, $sql)or die("Error: ".odbc_error($cid));
	while ($fila=odbc_fetch_array($resultado)){$ven[] = $fila;}	return $ven;
	}
	
	function ListarModeloDesM($cod){
	include('cn/dbconex.php');
	$sql="SELECT C_NUMITM, C_DESITM, C_ESTADO
			 FROM Dettabla WHERE C_CODTAB='MOD' AND C_ESTADO='1' and Dettabla.C_NUMITM='$cod' ";
$resultado = odbc_exec($cid, $sql)or die("Error: ".odbc_error($cid));
	while ($fila=odbc_fetch_array($resultado)){$ven[] = $fila;}	return $ven;
	}
