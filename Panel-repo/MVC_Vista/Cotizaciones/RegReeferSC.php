<?php 
$swequipo=$resulv3=$resu=$nomproveedor="";
if($swequipo
        =='1'){
	$des=$_GET['nom'];
	$c_codprd=$_GET['coprod'];
	$idequipo=substr($des,0,1).'-'.$_GET['c_codequipo'];
	}

if($resulv3!=NULL){
	
	foreach($resulv3 as $prov){
	echo $nomproveedor=$prov['PR_RAZO'];	
...
$c_tipgas	=$item['c_tipgas'];
	...
	$c_anno=$item['c_anno'];
	$c_control=$item['c_control'];
	$c_mcamaq=!empty($item['c_mcamaq'])?$item['c_mcamaq']:"";
	$pr_razo=!empty($item['pr_razo'])?$item['pr_razo']:"";
	$d_fecing=$item['d_fecing'];
	$d_fcrea=$item['d_fcrea'];
	$c_nronis=$item['c_nronis'];
	$c_codest=$item['c_codest'];
	$cod_prov=$item['c_codanx'];
	$c_anno=!empty($item['c_anno'])?$item['c_anno']:"";
	$proveedor=!empty($item['pr_razo'])?$item['pr_razo']:"";
$c_refnaci=$item['c_refnaci'];
$c_nacional=$item['c_nacional'];
	
$c_tipoio= !empty($item['c_tipoio'])?$item['c_tipoio']:"";     
$c_codcli= !empty($item['c_codcli'])?$item['c_codcli']:"";    
$c_nomcli= !empty($item['c_nomcli'])?$item['c_nomcli']:"";    
$c_nomcli2= !empty($item['c_nomcli2'])?$item['c_nomcli2']:"";   
$c_nomtec= !empty($item['c_nomtec'])?$item['c_nomtec']:"";    
$c_fecdoc= !empty($item['c_fecdoc'])?$item['c_fecdoc']:"";    
$c_placa1= !empty($item['c_placa1'])?$$item['c_placa1']:"";    
$c_placa2= !empty($item['c_placa2'])?$item['c_placa2']:"";    
$c_chofer= !empty($item['c_chofer'])?$item['c_chofer']:"";    
$c_fechora= !empty($item['pr_razo'])?$item['pr_razo']:"";   
$c_condicion= !empty($item['condi'])?$item['condi']:""; 
$c_tipois= !empty($item['c_tipois'])?$item['c_tipois']:"";    
$c_tipo= !empty($item['c_tipo'])?$item['c_tipo']:"";      
$c_tipo2= !empty($item['c_tipo2'])?$item['c_tipo2']:"";     
$c_tipo3= !empty($item['c_tipo3'])?$item['c_tipo3']:"";     
$c_obs= !empty($item['c_obs'])?$item['c_obs']:"";       
$c_combu= !empty($item['c_combu'])?$item['c_combu']:"";     
$c_usuario= !empty($_GET['udni'])?$_GET['udni']:"";   
$c_precinto= !empty($item['c_precinto'])?$item['c_precinto']:"";
$c_almacen= !empty($item['c_almacen'])?$item['c_almacen']:"";
$c_material= !empty($item['c_material'])?$item['c_material']:"";	
$c_numeir= !empty($item['c_numeir'])?$item['c_numeir']:"";
$c_tara= !empty($item['c_tara'])?$item['c_tara']:"";
$c_peso= !empty($item['c_peso'])?$item['c_peso']:"";
$c_cfabri= !empty($item['c_cfabri'])?$item['c_cfabri']:"";
$c_cmodel= !empty($item['c_cmodel'])?$item['c_cmodel']:"";

$c_idequipo= !empty($item['c_idequipo'])?$item['c_idequipo']:""; 
$c_codprd= !empty($item['c_codprd'])?$item['c_codprd']:""; 
$c_nserie= !empty($item['c_nserie'])?$item['c_nserie']:""; ;
$codmar= !empty($item['c_codmar'])?$item['c_codmar']:""; 
$c_codmod= !empty($item['c_codmod'])?$item['c_codmod']:"";
$c_codcol= !empty($item['c_codcol'])?$item['c_codcol']:""; 
$c_anno= !empty($item['c_anno'])?$item['c_anno']:""; 
$c_control= !empty($item['c_control'])?$item['c_control']:""; 
$c_codcat= !empty($item['c_codcat'])?$item['c_codcat']:""; 
$c_tiprop= !empty($item['c_tiprop'])?$item['c_tiprop']:""; 
$c_mcamaq= !empty($item['c_mcamaq'])?$item['c_mcamaq']:""; 
$c_procedencia= !empty($item['c_procedencia'])?$item['c_procedencia']:""; 
$c_nroejes= !empty($item['c_nroejes'])?$item['c_nroejes']:""; 
$c_tamCarreta= !empty($item['c_tamCarreta'])?$item['c_tamCarreta']:""; 
$c_serieequipo= !empty($item['c_serieequipo'])?$item['c_serieequipo']:""; 
$c_peso= !empty($item['c_peso'])?$item['c_peso']:""; 
$c_tara= !empty($item['c_tara'])?$item['c_tara']:""; 
$c_seriemotor= !empty($item['c_seriemotor'])?$item['c_seriemotor']:""; 
$c_canofab= !empty($item['c_canofab'])?$item['c_canofab']:""; 
$c_cmesfab= !empty($item['c_cmesfab'])?$item['c_cmesfab']:""; 
$c_cfabri= !empty($item['c_cfabri'])?$item['c_cfabri']:""; 
$c_cmodel= !empty($item['c_cmodel'])?$item['c_cmodel']:""; ;
$c_ccontrola= !empty($item['c_ccontrola'])?$item['c_ccontrola']:""; 
$c_tipgas= !empty($item['c_tipgas'])?$item['c_tipgas']:""; 
$procedencia= !empty($item['c_procedencia'])?$item['c_procedencia']:""; 
	}
}
  ?>
...
<!--es exactamente lo q a mi se me antojado-->
...
$().ready(function() {
	$("#c_cmodel").autocomplete("../MVC_Controlador/InventarioC.php?acc=seriemodeloauto", {
		width: 260, 
		matchContains: true,
		selectFirst: false
	});	
	$("#c_cmodel").result(function(event, data, formatted) {
		$("#c_cmodel").val(data[2]);
        ...
        <table width="796" border="0" align="left" cellpadding="0" cellspacing="0">
          <tr>
            <td colspan="4" align="left"> ()
              <input type="hidden" name="c_tipoio" id="c_tipoio"  value="<?php echo $tiping; ?>"/>
              <input type="hidden" name="swequipo" id="swequipo"  value="<?php echo $_GET['swequipo']?>"/></td>
          </tr>
          <tr>
            <td width="124">Descripcion</td>
            <td colspan="3"><input name="c_codprd" type="text" class="texto" id="c_codprd" value="<?php echo $des ?>" size="35" />
              Serie
              <input name="c_nserie" type="text" class="texto" id="c_nserie" value="<?php echo $idequipo ?>" size="20" />
...
<?php */?>
              ...
              <?php } ?>
              &nbsp;</td>
            <td colspan="2"><?php echo $c_codest ?>&nbsp;</td>
          </tr>
          <tr>
            <td bgcolor="#CCCCCC">DATOS CAJA</td>
            <td width="268" bgcolor="#CCCCCC">&nbsp;</td>
            <td colspan="2" bgcolor="#CCCCCC">DATOS MAQUINA REEFER</td>
          </tr>
          <tr>
            ...
            <td width="144">Año Fabricacion</td>
            <td width="260"><?php $ven = listaannoM();?>
              <select name="c_canofab" id="c_canofab" class="combo texto" >
                <option value="0">.::SELECCIONE::.</option>
                <?php foreach($ven as $item){?>
                <option value="<?php echo $item["tm_codi"]?>"<?php if($item["tm_codi"]==$c_canofab){?>selected<?php }?>><?php echo $item["tm_desc"]?></option>
                <?php }	?>
              </select></td>
          </tr>
          <tr>
            ...
            <td><?php $ven = listamesM();?>
              <select name="mfab1" id="mfab1" class="combo texto">
                <option value="0">.::SELECCIONE::.</option>
                <?php foreach($ven as $item){?>
                <option value="<?php echo $item["tm_codi"]?>"<?php if($item["tm_codi"]==$c_cmesfab){?>selected<?php }?>><?php echo $item["tm_desc"]?></option>
                <?php }	?>
              </select></td>
            <td>Mes Fabricacion</td>
            <td><?php $ven = listamesM();?>
              <select name="c_cmesfab" id="c_cmesfab" class="combo texto">
                <option value="0">.::SELECCIONE::.</option>
                <?php foreach($ven as $item){?>
                <option value="<?php echo $item["tm_codi"]?>"<?php if($item["tm_codi"]==$c_cmesfab){?>selected<?php }?>><?php echo $item["tm_desc"]?></option>
                <?php }	?>
          ...
          <tr>
            ...
            <td>Marca Maquina </td>
            <td><?php $ven = ListaMarcaMaqM();?>
              <select name="c_mcamaq" id="c_mcamaq"  class="combo texto">
                <option value="0">.::SELECCIONE::.</option>
                <?php foreach($ven as $item){?>
          ...
          <tr>
            <td>Fabricante</td>
            <td><input type="text" name="c_fabcaja" id="c_fabcaja" class="texto" value="<?php echo $c_fabcaja  ?>"  />
            <input type="hidden" name="c_seriemotor" id="c_seriemotor" /></td>
            <td>&nbsp;</td>
            <td><input type="hidden" name="c_cfabri" id="c_cfabri" class="texto" value="<?php echo $c_cfabri ?>" /></td>
          </tr>
          <tr>
            <td>Color</td>
          ...
          <tr>
            ...
            <td>Controlador</td>
            <td><input type="text" name="c_ccontrola" id="c_ccontrola" class="texto"  value="<?php echo  $c_ccontrola?>"/></td>
          </tr>
          <tr>
            <td colspan="2" bgcolor="#CCCCCC">DATOS PESO:</td>
            <td>Tipo Gas Refrigerante</td>
            <td><input name="c_tipgas" type="text"class="texto" id="c_tipgas" value="<?php echo $c_tipgas ?>" /></td>
          </tr>
          <tr>
            <td>Tara</td>
    ...
    <script type="text/javascript">
      ...
      <div class="TabbedPanelsContent">Proveedor:<?php echo $nomproveedor ?></div>
</div>
  </div>
  <input type="submit" name="button" id="button" value="GUARDAR INFORMACION" onclick="valida()"/>
</form>
<p>&nbsp; </p>
<p>&nbsp;</p>
<p>&nbsp;</p>
