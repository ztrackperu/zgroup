...
if(confirm(mensaje)){
location.href="../MVC_Controlador/AdminC.php?acc=eliminaracceso&id="+nro+"&udni="+dni;
 }else{
	 return false;
	}
...
<body>
<form id="form1" name="form1" method="post" action="../MVC_Controlador/AdminC.php?acc=veracceso&udni=<?php echo $val=$_GET['udni']; ?>&mod=<?php echo $_GET['mod'];?>&id=<?php echo $_GET['id'];?>">
<div class="onecolumn">
                    <div class="header"><strong>LISTA DE ACCESOS  &nbsp; <?php echo $id=!empty($_GET['id'])?$_GET['id']:""; ?></strong></div>
                    <strong style="color:#FFF"><?php echo $mod=$_GET['mod']; ?></strong>
<strong>Nuevo Acceso</strong>
<a href="../MVC_Controlador/AdminC.php?acc=nuevoacceso&udni=<?php echo $val=$_GET['udni']; ?>&mod=<?php echo $_GET['mod'];?>&id=<?php echo $val=$_GET['id']; ?>"><img src="../images/nuevo.png" width="40" height="40" /></a> 
...
<br class="clear"/>
                    <div class="content">
                        <table class="data" width="894" cellpadding="0" cellspacing="0" bordercolor="#000000">
                            <thead>
                                <tr>
                                  <th colspan="9" style="width:5%"><input type="radio" name="sw" id="sw" value="1" /> LOCAL
...
<!--<img src="../images/pdf.gif" alt="1" width="19" height="19" title="Ver Orden" />imprime&nbsp; -->
                                ...
                                <tr>
                                  ...
                                  <th width="15%"  >Local</th>
                                    <th width="15%"  >Nombre de la PC</th>
                                    <th width="10%" >Usuario</th>
                                    <th width="15%" >IP Publica</th>
                                    <th width="15%" >IP Local</th>
                                  <th width="10%" >MAC</th>
                                  <th width="15%" align="center" >Administrar</th>
										...
										if($listaos!=NULL)
{
            ...
            ?>
                                <tr>
                                  <td><?php echo $item2["correlativo"];?></td>
                                  <td bgcolor=""><?php echo utf8_encode($item2["local"]);?> </td>
                                  <td bgcolor=""><?php echo utf8_encode($item2["nombrepc"]);?> </td>
                                  <td bgcolor=""><?php echo $item2["usuario"];  ?></td>
                                  <td bgcolor="#FFFFFF"><?php echo $item2["ippublica"];  ?></td>
                                  <td bgcolor="#FFFFFF"><?php echo $item2["iplocal"];  ?></td>
                                  <td bgcolor="#FFFFFF"><?php echo $item2["mac"]?></td>
                                  <td bgcolor="#FFFFFF" align="center">
                                    
                                    
                                    <a href="../MVC_Controlador/AdminC.php?acc=updateacceso&udni=<?php echo $val=$_GET['udni']; ?>&mod=<?php echo $_GET['mod'];?>&id=<?php echo $item2["correlativo"];?>"> <img src="../images/icon_edit.png" alt="editar" width="18" height="18" class="help" title="Editar" /></a>&nbsp; &nbsp; 
                                    
                                    
                                    <a href="#" onclick="eliminaroc('<?php echo $item2["correlativo"];?>','<?php echo $_GET['udni'];?>')"><img src="../images/icon_delete.png" width="18" height="18" title="Eliminar" /></a> &nbsp;&nbsp; 
  ...
  <!--<a href="../MVC_Vista/Compras/Reportes/impoc.php?os="><img src="../images/pdf.gif" width="19" height="19" title="Imprimir Datos" /></a>&nbsp;-->
		...
		if ($pagxlet > 1) {
			...
			for ($i=0;$i<$pagxlet;$i++) {
         	if ($i==$pag){
				//echo $sel= ($i+1);
                                    $qws = !empty($_REQUEST['des'])?$_REQUEST['des']:"";
				echo "<a class='active' href='../MVC_Controlador/AdminC.php?acc=veracceso&udni=".$_GET['udni']."&mod=".$_GET['mod']."&des=".$qws."&sw=".$_REQUEST['sw']."&pag=".$i."'>".($i+1).'</a>';
			}else{
                            $qws = !empty($_REQUEST['des'])?$_REQUEST['des']:"";
			echo "<a href='../MVC_Controlador/AdminC.php?acc=veracceso&udni=".$_GET['udni']."&mod=".$_GET['mod']."&des=".$qws."&sw=".$_REQUEST['sw']."&pag=".$i."'>".($i+1).'</a>';
			}			
