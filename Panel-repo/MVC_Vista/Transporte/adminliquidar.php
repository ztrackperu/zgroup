...
$(document).ready(function(){
        ...
        $("#c_coddes").prop('disabled', true);        
        // Hacemos la lógica que cuando nuestro SELECT cambia de valor haga algo
        $("#c_idconcepto").change(function(){
			$("#c_descripcion").css("display", "none");
			$("#c_descripcion").val('');
			$("#c_coddes").css("display", "block");
            ...
            if($(this).val() != '')
            {
                $.ajax({
                    ...
                    error: function()
                    {
						 ...
						 //habilitamos c_descripcion
				 		 c_descripcion.prop('disabled', false);
				  		 c_descripcion.css("display", "block");
                    }
                });
            }
            else
            {
				  //habilitamos c_descripcion
				  c_descripcion.prop('disabled', false);
				  c_descripcion.prop('hidden', false);
...
function abrirmodalUpdLiquidar(Id_liquidetviatico,n_item,Id_servicio){
	    ...
	    $('#tablaUpdLiquidar').load("?c=01&a=VerUpdLiquidar",{Id_liquidetviatico:Id_liquidetviatico,n_item:n_item,Id_servicio:Id_servicio});	
...
function validarguardar(){
	  ...
	  //var c_tipdoc=document.getElementById('c_tipdoc').value;
 	  else if((document.RegLiquidar.c_tipdoc.value)=='SELECCIONE'){			
	 ...
	 // var c_seriedoc=document.getElementById('c_seriedoc').value;
 	   else if((document.RegLiquidar.c_seriedoc.value)==""){		
 ...
 function validarguardarupd(){
	  ...
	  //var c_tipdoc=document.getElementById('c_tipdoc').value;
 	  else if((document.UpdLiquidar.c_tipdoc.value)=='SELECCIONE'){			
			var mensje = "Falta Seleccionar el Tipo del Documento ...!!!";
				alert(mensje);
			//document.getElementById("c_tipdoc").focus();
			return 0;
	 ...
	 // var c_seriedoc=document.getElementById('c_seriedoc').value;
 	   else if((document.UpdLiquidar.c_seriedoc.value)==""){		
			var mensje = "Falta Ingresar la Serie del Documento ...!!!";
				alert(mensje);
			document.getElementById("c_seriedoc").focus();
			return 0;
...
<div class="modal fade" id="my_modalelim" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">  
       ...
       <form id="frm-eliminarAsig" class="form-horizontal" method="post" action="?c=01&udni=<?php echo $_GET['udni']?>&mod=<?php echo $_GET['mod']?>&a=EliminarLiquidacion" >
       ...
       <h5 class="modal-title">Mensaje del Sistema</h5>             
        ...
        <div class="modal-body" id="exampleModalLabel"> 
			...
			¿Seguro de Eliminar la liquidación del Item <input name="n_item" id="n_item" type="text" style="border: 0px solid #A8A8A8;width:20px;text-align:center;" readonly />  del Viatico
...
<form class="form-horizontal" id="RegLiquidar" name="RegLiquidar" method="post"  action="?c=01&udni=<?php echo $_GET['udni']?>&mod=<?php echo $_GET['mod']?>&a=GuardarLiquidacion">
<div class="modal fade" id="my_modalRegLiquidar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" >
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">      
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="exampleModalLabel">Nuevo Detalle de Liquidacion<input name="empresa" id="empresa" type="text" style="border: 0px solid #A8A8A8;width:300px;" readonly /></h4>
      </div>
      	<div class="modal-body">   
            ...
            <div class="form-group"	>
                ...
                <div class="col-xs-4">
                    ...
                    <select name="c_coddes" id="c_coddes" class="form-control" onBlur="cambiarpeaje()" onChange="cambiarpeaje()">
                    </select>
                </div>  
...
<form class="form-horizontal" id="UpdLiquidar" name="UpdLiquidar" method="post"  action="?c=01&udni=<?php echo $_GET['udni']?>&mod=<?php echo $_GET['mod']?>&a=GuardarUpdLiquidacion">
<div class="modal fade" id="my_modalUpdLiquidar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">      
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="exampleModalLabel">Editar Detalle de Liquidacion<input name="empresa" id="empresa" type="text" style="border: 0px solid #A8A8A8;width:300px;" readonly /></h4>
      </div>
...
<?php /*?><form class="form-horizontal" id="frmliquidar" name="frmliquidar" method="post"  action="?c=01&udni=<?php echo $_GET['udni']?>&mod=<?php echo $_GET['mod']?>&a=GuardarUpdLiquidacion" ><?php */?>
    <div class="form-control-static" align="right">
     ...
     &nbsp;<a class="btn btn-danger" href="?c=01&udni=<?php echo $_GET['udni']?>&mod=<?php echo $_GET['mod']?>&a=ListarViaticos&Id_servicio=<?php echo $Id_servicio ?>&n_item=<?php echo $n_item ?>&Id_viatico=<?php echo $Id_viatico ?>">Cancelar</a>
     <!--&nbsp;<a class="btn btn-warning" onClick="location.reload();" >Refrescar</a>&nbsp;-->
     &nbsp;<a class="btn btn-warning" href="?c=01&udni=<?php echo $_GET['udni']?>&mod=<?php echo $_GET['mod']?>&a=ListarViaticos&Id_servicio=<?php echo $Id_servicio ?>&n_item=<?php echo $n_item ?>&Id_viatico=<?php echo $Id_viatico ?>">Regresar</a>
	...
	?>
         <tr>
          ...
          <!--FIN RECUPERAR PARA VOLVER -->
           ...
           <!--FIN RECUPERAR PARA GUARDAR -->
           <select id="" name=""  class="form-control input-sm" disabled>
              <option value="SELECCIONE">SELECCIONE</option>
              <?php 
			  	$conceptoviaticos=$this->model->ListarConceptoViaticos(); 
			    foreach ($conceptoviaticos as $convia){
			  ?>
              <option value="<?php echo $convia->C_NUMITM; ?>" <?php if($convia->C_NUMITM==$r->c_idconcepto){?>selected<?php }?>><?php echo $convia->C_DESITM; ?></option>
              <?php } ?>
              </select>   
              </td>           
             <td><?php echo $r->c_descripcion; ?></td>          
          <td>	<?php echo $r->n_impogast;  ?> </td>         
          <td>                
              <?php 
          ...
          <td><?php echo $r->c_seriedoc; ?> </td>
          <td><?php echo $r->c_numdoc; ?></td>
          <td><?php echo vfecha(substr($r->d_fecdoc,0,10)); ?></td>         
       	  <td> 
          <div class="dropdown">
              ...
              <ul class="dropdown-menu" role="menu">  
              	  <li><a onClick="abrirmodalUpdLiquidar('<?php echo $r->Id_liquidetviatico ?>',<?php echo $n_item ?>,'<?php echo $Id_servicio ?>')">Editar</a></li> 	            	               	   
                  <li><a onClick="abrirmodal('<?php echo $r->Id_liquidetviatico ?>',<?php echo $n_item ?>,'<?php echo $Id_servicio ?>','<?php echo $Id_viatico ?>','<?php echo $Id_detviatico ?>')" >Eliminar</a></li>
