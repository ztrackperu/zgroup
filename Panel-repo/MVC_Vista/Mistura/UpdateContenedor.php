<?php 
if($obtenercontenedor!=NULL)
{
	foreach ($obtenercontenedor as $item)
	{
  $id_contenedor =$item['id_contenedor'];
  $descripcion_contenedor =$item['descripcion_contenedor'];
  $placa_contenedor  =$item['placa_contenedor'];
  $estado =$item['estado'];
...
<form name="form1" id="form1" method="post" 
 action="../MVC_Controlador/MisturaC.php?acc=m3">
<table width="531" border="0">
  <tr>
    <td style="color:#039">MODIFICACION DATOS DE CONTENEDORES</td>
  </tr>
</table>
...
<table width="532" border="0">
  <tr>
    ...
    <td width="518"><input name="textfield"  class="texto" type="text" id="textfield" value="<?=$descripcion_contenedor?>" size="25">
      <input type="hidden" name="hiddenField" id="hiddenField" value="<?php echo $id_contenedor?>">
      <input type="hidden" name="hiddenField2" id="hiddenField2" value="<?php echo $_GET['udni'];?>"></td>
  </tr>
  <tr>
    <td width="159">Placa</td>
    <td><input name="textfield2"  class="texto" type="text" value="<?=$placa_contenedor?>" size="25"></td>
  </tr>
  <tr>
