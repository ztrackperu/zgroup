...
$pdf->AddPage();
$cabecera="SELECT cabos.c_numos as os,cabos.*,detos.*, dt1.c_desitm as trato_pago, tp.tp_desc as factura_pago
from (((cabos
inner join detos on cabos.c_numos = detos.c_numos)
inner join dettabla dt1 on dt1.c_numitm = cabos.c_tratopag)
inner join tab_pago tp on tp.tp_codi = cabos.c_codpgf)
where cabos.c_numos = '$c_numos'
and dt1.c_codtab='CPO' AND dt1.C_ESTADO='1'
and tp.tp_esta='1'";
...
$imagprd=isset($fila['c_codtit'])?$fila['c_codtit']:"";
$c_numos=$fila['os'];
...
$pdf->Ln(30);
//
$variable2 = explode ('-',substr($fila['d_fecre'],0,10)); //división de la fecha utilizando el separador -
...
$pdf->SetTextColor(0,0,0);
$pdf->Cell(2,7,'RAZON SOCIAL   :'.utf8_decode($fila['c_nomprov']),0,1,'L'); 
...
$i=1;
$c_swfirma=isset($resultado['c_swfirma'])?$resultado['c_swfirma']:"";
$zglos=isset($resultado['c_descr2'])?utf8_decode($resultado['c_descr2']):"";
...
if(isset($resultado['c_tipped']) && $resultado['c_tipped']=='017' && $resultado['c_codcont']!=NULL){
	$gl='SERV. ALQUILER';
	...
	}elseif(isset($resultado['c_codcont']) && $resultado['c_codcont']!=NULL){
	$gl='';	
...
$pdf->SetFillColor(204,202,201);
$pdf->SetTextColor(0,0,0);
$pdf->cell(90,7,'DETALLE DE PAGO: '.$fila['trato_pago'],1,0,'L');
$pdf->cell(90,7,'SUB TOTAL'.' :'.$xsbt,1,0,'R');
$pdf->Ln(); 
$pdf->SetTextColor(0,0,0);
$pdf->cell(90,7,'CONDICION : '.$fila['factura_pago'],1,0,'L');
...
- EL NÚMERO DE LA PRESENTE ORDEN DEBE FIGURAR EN SU FACTURA, GUÍAS Y CERTIFICADOS.<br />
- PARA LA ACEPTACIÓN DE SU  FACTURA DEBERÁ ADJUNTAR COPIA  DEL PRESENTE  DOCUMENTO,
GUIAS Y/O CERTIFICADOS   CORRESPONDIENTES.<br /> ZGROUP  NO SE RESPONSABILIZA POR
DEMORA EN EL PAGO POR ERROR DOCUMENTARIO POR   PARTE DEL PROVEEDOR.<br />
- PARA ENTREGA DE SU MERCADERÍA O PRESTACIÓN DE SERVICIO ES  INDISPENSABLE LA
PRESENTACION DEL PRESENTE DOCUMENTO.<br />
-RECEPCION DE FACTURAS TODOS LOS JUEVES DE 2PM A 5PM. <br />
-LUGAR RECEPCION:OFICINA ADMINISTRATIVA (Cal. Ordoner Vargas 142 VillasSol - Los Olivos)'));
