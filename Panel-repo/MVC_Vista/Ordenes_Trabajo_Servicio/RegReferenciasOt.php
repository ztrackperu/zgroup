...
<body>
<form id="formElem" name="formElem" method="post" action="../MVC_Controlador/OrdenTrabajoC.php?acc=guardarefot&udni=<?php echo $_GET['udni'] ?>">
	<?php
	
        if($resulos!=NULL)
            {
				foreach ($resulos as $itemT) 
				{	
					$c_tipdoc=$itemT["c_tipdoc"];
					$c_serdoc=$itemT["c_serdoc"];
					$c_nrodoc=$itemT["c_nrodoc"];
					if($itemT["c_tipdoc"]!=NULL){
						$alerta='NOTA: La orden ya tiene una '.$_GET['fact'].' referencia asignada ';
					}else{
						$alerta='La orden NO tiene una '.$_GET['fact'].' referencia asignada ';
					}
				}					
			}else{
				$c_tipdoc='';
  ...
  <div class="column_left">
    <div class="header"><h2><strong>REFERENCIA ORDEN TRABAJO</strong></h2></div>
  ...
  <table width="527" border="0" cellpadding="0" cellspacing="0">
    <tr>
      ...
      <td valign="middle">
        <input type="text" name="c_numot" id="c_numot"  class="texto" value="<?php echo $_GET['ot'] ?>"/>
        <input type="hidden" name="c_usrudp" id="c_usrudp" value="<?php echo $_GET['udni'] ?>" /></td>
        <input type="hidden" name="fact" id="fact" value="<?php echo $_GET['fact'] ?>" /></td>
    </tr>
    ...
    <tr>
      ...
      <td valign="middle"><label for="textfield"></label>
       <input name="c_serdoc" value='<?php echo $c_serdoc ?>' type="text" required="required" class="texto"  id="c_serdoc" onblur="ponerCeros(this)"  size="10" maxlength="3"/></td>
    </tr>
    <tr>
      ...
      <td valign="middle">
        ...
        <input name="c_nrodoc" value='<?php echo $c_nrodoc ?>' type="text" required="required" class="texto" id="c_nrodoc" onblur="ponerCeros2(this)" size="35" maxlength="7"/></td>
    </tr>
    <tr>
