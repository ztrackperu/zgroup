...
//$razon=strtoupper(utf8_decode($xrazon));
$cc_ndni=' ';
...
$cc_ruc='auotgenerado';
$cc_razo=strtoupper(utf8_decode($razon));$cc_nruc=$cc_nruc;

$cc_dir1=strtoupper(utf8_decode($_GET['d1']));$cc_dir2=strtoupper(utf8_decode($_GET['d2']));$cc_cdis=$_GET['dis'];
$cc_tele=$_GET['fono'];$cc_tfax='';$cc_emai=strtoupper($_GET['ema']);$cc_cven='000';$cc_czon='000';$cc_resp=strtoupper(utf8_decode($_GET['con']));$cc_pago='00';$cc_ruca='';$cc_noma='';$cc_dira='';$cc_tela='';$cc_freg=date("d/m/Y");$cc_cate='01';$cc_swmon='1';$cc_ccul='000';$cc_ccan='00';$cc_ncom=strtoupper(utf8_decode($razon));$cc_tcli=$_GET['tip'];$cc_tdoc=$_GET['doc'];
...
$cc_ruc=$_GET['cli'];$cc_razo=strtoupper(utf8_decode($razon));$cc_nruc=$cc_nruc;$cc_dir1=strtoupper(utf8_decode($_GET['d1']));$cc_dir2=strtoupper(utf8_decode($_GET['d2']));$cc_cdis=$_GET['dis'];
$cc_tele=$_GET['fono'];$cc_tfax='';$cc_emai=$_GET['ema'];$cc_cven='000';$cc_czon='000';$cc_resp=strtoupper(utf8_decode($_GET['con']));$cc_pago='00';$cc_ruca='';$cc_noma='';$cc_dira='';$cc_tela='';$cc_freg=date("d/m/Y");$cc_cate='01';$cc_swmon='1';$cc_ccul='000';$cc_ccan='00';$cc_ncom=strtoupper(utf8_decode($cc_ncom));$cc_tcli=$_GET['tip'];$cc_tdoc=$_GET['doc'];
$CC_FCREA=date("d/m/Y H:i:s");
$cc_evta='00';$cc_esta='1';$cc_situd='C';$cc_lfer='0';$cc_ccor=' ';$cc_TOPE='';$cc_LCFER='0';$cc_FVLC='0';$cc_PAGOF='0';$cc_MBLOQ='0';$cc_IVTA='0';$cc_ARET='0';$cc_ICOB='0';$cc_DLFER='0';$cc_DLAGRO='0';$cc_SWINT='0';$cc_FVLCF='0';$c_codcia='01';$c_codtda='000';
$cc_pagof='00';
$cc_lsol='0';$cc_lnc='0';
...
/*$mensaje="Este Modulo Ya no se encuentra Disponible en esta versión";
		print "<script>alert('$mensaje')</script>";*/
}
...
if($_GET['acc']=='updateitemcobro'){
	...
	$coti=$_REQUEST['coti'];
	$cli=$_REQUEST['cli'];
	UpdatecronogramalimpiaiteC($id,$coti);
	for($i=1;$i<=200;$i++){
	$id=$_REQUEST['sel'.$i];
	$finicio=$_REQUEST['finicio'.$i];
	$ffin=$_REQUEST['ffin'.$i];
	 
	if($id!='')	{ //pregunto si el check esta activo
...
if($sw=='1'){
$reporte=Listar_CotizacionesM($cli,$ff,$sw,$mod,$oper);
	//$reporte2=Listar_CotizacionesAprobadasC($cli,$ff,$sw);
	}
	if($sw=='2'){
		
$reporte=listar_CotizacionesAsuntoC($asunto,$mod,$oper);
		}
	if($sw=='3'){
		
		//echo $cot;
$reporte=Listar_CotizacionesxNroM($coti,$ff,$sw,$mod,$oper);
...
$reporte=Listar_CotizacionesForwarderM($forwarder,$mod,$oper);	
...
if($_GET["acc"] == "c01") // MOSTRAR: Formulario Nuevo Registro
{
	...
	if($listcontweb!=NULL){//Lista_Cotizacion
...
function listar_CotizacionesAsuntoC($asunto,$mod,$oper){ return listar_CotizacionesAsunto($asunto,$mod,$oper);}
 ...
 if($busquedapacienteauto!=NULL)
{
	foreach ($busquedapacienteauto as $item)
	{
		...
		$telefono = utf8_encode($item["pr_tele"]);
		$email = utf8_encode($item["pr_emai"]);
		$contacto = utf8_encode($item["pr_resp"]);

		echo "$decod|$cod|$razon|$con1|$con2|$con3|$email|$telefono|$contacto\n";
				//0		1		2	3 array
		//NOTA: la primara variable esta concatenado el $paterno , cada uno de ellos formar un arrar 
	}
}
...
if($c_precios=='001'){
	...
	}else if($c_precios=='002'){
			$b_inlgv=1;
		
		}else if($c_precios=='003'){
			$b_inlgv=2;
		}
...
$cuentaconcrono=buscacoticoncronogramaM($nrocotizacion);
...
UpdateCotizacionCabC($nrocoti,$fpag,$d_fecvig,$useraprob,$d_fecapr,$c_numocref);
...
$sql="update pedi_cronograma set c_swcob='0',c_nrofac='' where c_nroped='$znrocoti' and c_estado<>'4'";
	@$resultado = odbc_exec($cid, $sql)or die(exit("Error en odbc_exec()<br>$sql"));
		    }
		 }
	   }
...
function UpdateCotizacionCabC($nrocoti,$fpag,$d_fecvig,$useraprob,$d_fecapr,$c_numocref){ $resul= UpdateCotizacionCabM($nrocoti,$fpag,$d_fecvig,$useraprob,$d_fecapr,$c_numocref);}
...
{	
$PR_RUC=$_REQUEST['ruc'];
$PR_RAZO=ltrim(strtoupper(utf8_decode($_POST['textfield3'])));
$PR_NRUC=$_POST['ruc'];
$PR_DIR1=ltrim(strtoupper(utf8_decode($_POST['textfield5'])));
$PR_DIR2=ltrim(strtoupper(utf8_decode($_POST['textfield5'])));
...
$PR_RUC=$_REQUEST['ruc'];
$PR_RAZO=ltrim(strtoupper(utf8_decode($_REQUEST['textfield3'])));
$PR_NRUC=$_REQUEST['ruc'];
$PR_DIR1=ltrim(strtoupper(utf8_decode($_REQUEST['textfield5'])));
$PR_TELE=$_REQUEST['textfield7'];
$PR_EMAI=$_REQUEST['textfield8'];

$PR_FREG=date("d/m/Y h:m:s");
$PR_OPER=$_REQUEST['user'];
actualizaprovM($PR_RUC,$PR_RAZO,$PR_DIR1,$PR_TELE,$PR_EMAI,$PR_OPER,$PR_FREG);
...
if($_GET["acc"] == "datosmanufactura") // MOSTRAR: Formulario Nuevo Registro
{
	...
	$tipo=$_GET['codclase'];
		...
		if($tipo=='001' || $tipo=='015'){ //muestra formulario DRY Y MODULOS
...
$c_nacional=$_REQUEST['radio2'];
 ...
 $c_ccontrola=$_REQUEST['c_ccontrola'];
...
if($_GET["acc"] == "adicionatransportista")
{
	...
	$obtenerproveedor=obtenerprovedorM($ruc);
...
if($_GET["acc"] == "cargar_ocompra") // MOSTRAR: Formulario Nuevo Registro
{
	$texto =strtoupper($_REQUEST["q"]);
 //SELECT C_NUMPED,C_CODCLI,C_NOMCLI,C_CONTAC,C_TELEF,C_NEXTEL,C_EMAIL,C_ASUNTO,CC_NRUC,CC_DIR1 
 $busquedapacienteauto = listaocompraM($texto);
 if($busquedapacienteauto!=NULL)
{
	foreach ($busquedapacienteauto as $item)
	{
		$guianum=$item["c_numeoc"];
		$ncli=$item["c_nomprv"];
		$ccli=$item["c_codprv"];
		$decod=$guianum.' '.$ncli;
		echo "$decod|$guianum|$ncli|$ccli\n";
				//0		1		2	3 array
		//NOTA: la primara variable esta concatenado el $paterno , cada uno de ellos formar un arrar 
	}
}
...
if($_GET["acc"] == "verdetalleequi") 
{	
   // $situacion=$_GET['situ'];
	$idequipo=$_GET['cod'];		
	//$reporte=buscaequipocliente2C($idequipo);
	$reportehistorialequipo=buscahistorialequipoM($idequipo);
	$reporteguiarem=buscaequipoguiaremM($idequipo);
	$reporteeir=buscaequipoeirM($idequipo);
	
	include('../MVC_Vista/Cotizaciones/detallesituequipo.php');
	
}
if($_GET["acc"] == "reporte9") 
{
	include('../MVC_Vista/Cotizaciones/reporte9.php');
}


if($_GET["acc"] == "verrep09") 
{
	$coti=$_POST['ncoti'];
	$valida=buscacotixosM($coti);
	if($valida!=""){
		$reporte=buscacotixosM($coti);
	include('../MVC_Vista/Cotizaciones/reporte9.php');
	}else{
		print "<script>alert('No Hay Referencias de Ordenes Servicio')</script>";	
		include('../MVC_Vista/Cotizaciones/reporte9.php');
		}
}
?>