...
{	
$c_numoc=$_GET['oc'];
$resultado=BusqxOc($c_numoc);
include("../MVC_Vista/Compras/RegReferencias.php");
}

if($_GET["acc"] == "verfrmref2") // MOSTRAR: Formulario auto busqueda de proveedor
{	
$c_numoc=$_GET['oc'];
$resultado=BusqxOc($c_numoc);
include("../MVC_Vista/Compras/RegReferencias2.php");
}

if($_GET["acc"] == "guardarefoc") // MOSTRAR: Formulario auto busqueda de proveedor
{	
	$c_numoc=$_REQUEST['c_numoc'];
	$c_tipmon=$_REQUEST['c_tipmon'];
	$c_importe=$_REQUEST['c_importe'];
	...
	UpdateReferenciaOCM($c_numoc,$c_tipmon,$c_importe,$c_tipdoc,$c_serdoc,$c_nrodoc,$c_usrudp,$d_fecudp);
...
if($_GET["acc"] == "guardarefoc2") // MOSTRAR: Formulario auto busqueda de proveedor
{	
	$c_numoc=$_REQUEST['c_numoc'];
	$c_tipmon=$_REQUEST['c_tipmon'];
	$c_importe=$_REQUEST['c_importe'];
	...
	$d_fecudp=date("d/m/Y H:i:s");
	$c_usrudp=$_REQUEST['c_usrudp'];
	UpdateReferenciaOCM2($c_numoc,$c_tipmon,$c_importe,$c_tipdoc,$c_serdoc,$c_nrodoc,$c_usrudp,$d_fecudp);
		$mensaje="Se Actualizo Referencia......";
	print "<script>alert('$mensaje')</script>";
	$sw="1";
	$descripcion='';
 ...
 if($busquedapacienteauto!=NULL)
{
	foreach ($busquedapacienteauto as $item)
	{
		...
		echo "$decod|$cod|$razon|$con1|$con2|$con3|$con5|$email|$telefono|$contacto\n";
 //0	  1		2	3      array
		//NOTA: la primara variable esta concatenado el $paterno , cada uno de ellos formar un arrar 
	}
}
...
{	
	...
	$c_pais=$_REQUEST['c_pais'];
	$c_icote=$_REQUEST['c_icote'];
	$c_costo=$_REQUEST['ccosto'];
	$monto=isset($_REQUEST['txtMonto'])?$_REQUEST['txtMonto']:'0';
	$c_puerto=isset($_REQUEST['c_puerto'])?$_REQUEST['c_puerto']:'';
	$c_depo=isset($_REQUEST['c_depo'])?$_REQUEST['c_depo']:'';
	...
	for($i=0;$i<=100;$i++){
		if(isset($_REQUEST['codigo'.$i])){
			...
			if($c_codprd != ""){
				...
				//inicia solo si es registro duplicado
				$c_numeocant=isset($_REQUEST['c_numeocant'])?$_REQUEST['c_numeocant']:"";	
				if(trim($c_nroserie)!="" && trim($c_numeocant)!=""){
					deletedetduplicadoM($c_nroserie,$c_numeocant);
					$d_feaprb=date("d/m/Y H:i:s");
					$c_uaprb=$_REQUEST['c_oper'];
					//$c_numeoc=$c_numeoc;
					$c_obaprb='OC duplicado. Aprobacion automatica';
					ApruebaosM($c_numeoc,$c_uaprb,$d_feaprb,$c_obaprb);		
				}
				//fin solo si es registro duplicado
			}
		}
	}//end
...
function TotalPedidoC($n_bruafe,$n_totoc1,$n_totoc2,$n_totoc3,$c_numeoc){
	$resu=TotalPedidoM($n_bruafe,$n_totoc1,$n_totoc2,$n_totoc3,$c_numeoc);
}
...
$c_pais=$_REQUEST['c_pais'];
$c_icote=$_REQUEST['c_icote'];
$c_puerto=$_REQUEST['c_puerto'];
$c_depo=$_REQUEST['c_depo'];
$c_costo=$_REQUEST['ccosto'];
$monto=$_REQUEST['txtMonto'];

		
UpdateCabComprasM($c_numeoc,$d_fecmod,$c_opermd,$n_tcoc,$c_codmon,$c_codpag,$c_codlug,$c_deslug,$c_estado,$c_obsoc,$d_fecent,
$c_lugent,$c_codtran,$c_nomtran,$c_tipooc,$c_refcoti,$c_pais,$c_icote,$c_puerto,$c_depo,$c_costo,$monto);	
