...
if($_GET["acc"] == "regotra") // MOSTRAR: Formulario Nuevo Registro 
{
	...
	if($lista!=NULL){
		...
		$mensaje="Por favor Cerrar sus OT // Tiene $cantOTG Ordenes de Trabajo por cerrar";
...
if($_GET["acc"] == "buscarOtra") // MOSTRAR: Formulario Nuevo Registro
{
	$udni=$_GET['udni'];
	$resultado=Ver_usuarioC($udni);
	$descripcion=$_REQUEST['des'];
...
function ListaCentroCostoC(){return CentroCostoM();}
...
if($_GET["acc"] == "guardaos"){
	...
	$c_unidad=$_REQUEST['c_unidad'];
	$c_kilometraje=$_REQUEST['c_kilometraje'];
	$c_costo=$_REQUEST['ccosto'];
	$c_estado='0';
	...
	$c_rh=$_REQUEST['c_rh'];
	$tip_igv=$_REQUEST['tip_igv'];
	$ocompra=$_REQUEST['c_refcoti2'];
...
if($_GET["acc"] == "AgregarDocumentos") // MOSTRAR: Formulario Nuevo Registro
{
	$udni=$_GET['udni'];
	$resultado=Ver_usuarioC($udni);
	$ot=$_REQUEST['cod_ot'];
	...
	$ubicacion="archivos\orden_trabajo";
		...
		if (($_FILES['AddArchivos']['name'] == !NULL)) 
				{
					...
					$NomImagenR=$ubicacion."/".$ot.'.'.$tipoArchivo[1];		
					...
					echo $NomImagenR;
					AgregarDocumentosM($ot,$NomImagenR);
...
$nro_guia=strtoupper($_REQUEST['nro_guia']);
$nro_ticket=strtoupper($_REQUEST['nro_ticket']);
$c_numeroReporte=strtoupper($_REQUEST['c_numeroReporte']);$c_serieEquipo=strtoupper($_REQUEST['c_serieEquipo']);$c_desequipo=strtoupper($_REQUEST['c_desequipo']);$unidad=strtoupper($_REQUEST['codigoequipo']);$d_fecdcto=$_REQUEST['d_fecdcto'];$c_codmon=$_REQUEST['xc_codmod'];$c_treal=strtoupper($_REQUEST['c_treal']);$c_asunto=strtoupper($_REQUEST['c_asunto']);
...
ActualizaEstadoEQM($unidad,$c_numot);
...
$add1=strtoupper($_REQUEST['c_tratopag']);
$add2=strtoupper($_REQUEST['c_codpgf']);
...
for($i=1;$i<=20;$i++){
	...
	//	$reporteot=CargaOrdenTrabajoM($c_numot);$_GET['udni']
...
if($_GET["acc"] == "veroTra2") // MOSTRAR: Formulario Modificar Registro
{
	$c_numot=$_GET['ot'];
	...
	$reportens=ListarNotasSalidasM($c_not);
	$totalNotas=ListarNotasSalidasTotalM($c_not);
		//echo $c_not;
include('../MVC_Vista/Ordenes_Trabajo_Servicio/ImprimirOT2.php');
}
...
if($_GET["acc"] == "eliminaotrab") // MOSTRAR: Formulario Nuevo Registro
{
	$c_numot=$_GET['ot'];
	$c_idequipo=$_GET['id'];
	ZActualizaEstadoEQM($c_idequipo,$c_numot);
...
if($_GET["acc"] == "CerrarOTrab") // MOSTRAR: Formulario Nuevo Registro
{
	 ...
	 $facturaprov=$_REQUEST['facturaprov'];
	 $h_fin=$_REQUEST['h_fin'];
	 cerrarOTM($c_numot,$d_feccierra,$c_usrcierra,$facturaprov,$h_fin);
...
if($_GET["acc"] == "verfrmrefot") // MOSTRAR: Formulario auto busqueda de proveedor
{	
$c_ot=$_REQUEST['ot'];
$ref=$_REQUEST['fact'];
$resulos=BusquedaRefOt($c_ot,$ref);
include("../MVC_Vista/Ordenes_Trabajo_Servicio/RegReferenciasOt.php");
}
...
{	
	...
	UpdateReferenciaOSM2($fact,$c_numos,$c_tipdoc,$c_serdoc,$c_nrodoc,$c_usrudp,$d_fecudp);
...
if($_GET["acc"] == "guardarefot") // MOSTRAR: Formulario auto busqueda de proveedor
{	
	$fact=$_REQUEST['fact'];
	$c_numot=$_REQUEST['c_numot'];
	$c_tipdoc=$_REQUEST['c_tipdoc'];
	...
	$d_fecudp=date("d/m/Y H:i:s");
	$c_usrudp=$_REQUEST['c_usrudp'];
	UpdateReferenciaOT($fact,$c_numot,$c_tipdoc,$c_serdoc,$c_nrodoc,$c_usrudp,$d_fecudp);
		$mensaje="Se Actualizo Referencia......";
	print "<script>alert('$mensaje')</script>";
	$udni=$_GET['udni'];
	$resultado=Ver_usuarioC($udni);
...
// DUPLICARRR
if($_GET["acc"] == "duplicaot") // MOSTRAR: Formulario DUPLICANDO LOS DATOS DE LA OC SELECCIONADO
{
	$c_numot=$_GET['ot'];
	$udni=$_GET['udni'];
	$resultado = Ver_usuarioC($udni);
	$reporteot = ListaDuplicaOT($c_numot);
	include('../MVC_Vista/Ordenes_Trabajo_Servicio/DuplicarOTra.php');
}

if($_GET["acc"] == "RegDuplicaOt") // MOSTRAR: Formulario Modificar Registro
{
