...
$validaProducto=BusquedasiexisteProductoM($c_codprd,$c_catprd,$c_conprd);
	...
	$c_mcamaq=$_REQUEST['c_mcamaq'];
	$c_cmodel=$_REQUEST['c_cmodel'];
	UpdPrecioNewM($c_numpre,$c_mcamaq,$c_cmodel);
...
$c_mcamaq=$_REQUEST['c_mcamaq'];//new
$c_cmodel=$_REQUEST['c_cmodel'];//new
$validaProductoPrecio=BusquedasiexisteProductoPrecioM($c_codprd,$c_catprd,$c_conprd,$n_preprd,$c_codmon,$c_mcamaq,$c_cmodel);
  ...
  if($validaProductoPrecio!=""){
	  ...
	  <script type="text/javascript">
					...
					if (ventana==true) 
					{ 
						location.href="PrecioC.php?acc=val&c_codprd=<? echo $c_codprd; ?>&c_catprd=<? echo $c_catprd; ?>&c_conprd=<? echo $c_conprd; ?>&c_opermod=<? echo $c_opermod; ?>&d_fecmod=<? echo $d_fecmod; ?>&n_preprd=<? echo $n_preprd; ?>&c_mcamaq=<? echo $c_mcamaq; ?>&c_cmodel=<? echo $c_cmodel; ?>";
...
if($_GET["acc"] == "ReporteGeneral") 
	{
		$udni=$_GET['udni'];
		$resultado1=Obtener_UsuarioM($udni);	
		include('../MVC_Vista/Precio/ReporteGeneral.php');
	}
	
	 if($_GET["acc"] == "listaGeneral")
{
...
//$cli=$_REQUEST["codcli"];
//$reporte=reporteguiasC($fecha1,$fecha2,$cli);
$reporte=listahistorialGeneralM($fecha1,$fecha2,$sw);
